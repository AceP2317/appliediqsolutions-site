...
/* ============================================================================
   ...
   Pick an intent; the fields adapt. The centerpiece is four narrow questions
   ...
   IT USED TO BE ONE WIDE QUESTION and that is why the inquiries were useless.
...
const ALL_INTENTS = [
  ...
  { key: 'assessment', label: 'Where does AI fit?', blurb: 'You want to use it and cannot see where. I look at how the work runs, then write it down.' },
...
/* THE SHORTEST ANSWER THAT IS STILL AN ANSWER, in characters.
   ...
   EFFORT, not a quality judgment: the message it produces says what is missing
...
export default function ContactForm() {
  ...
  const [form, setForm] = useState({
    ...
    /* FOUR NARROW QUESTIONS REPLACED ONE WIDE ONE, 2026-09-03.
       ...
       WHAT WAS WRONG. The single required field was a textarea labeled "The
       ...
       so the honest answer to that question is a short one — and the inquiries
  ...
  const buildSummary = () => {
    ...
    if (WANTS_A_TIME.has(intent)) {
      ...
      if (form.slot1) L.push('', `First choice: ${summarizeSlot(form.slot1)}`);
      if (form.slot2) L.push(`Second choice: ${summarizeSlot(form.slot2)}`);
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-7" noValidate>
      ...
      {/* ── THE CENTERPIECE, now four narrow questions ─────────────────────
