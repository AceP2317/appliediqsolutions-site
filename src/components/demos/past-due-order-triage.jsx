...
// The shared industrial faceplate. One place holds the seven demos' identity,
// and scripts/check-contrast.mjs reads that same module every build.
...
// ============================================================================
// THEME — the industrial faceplate, "demand & planning" band. Deepest chassis
// of the three, most room per row, condensed labels: a planning board.
//
// NO LIGHT MODE, AND THAT DELETED A DEFECT RATHER THAN REPAIRING ONE. Six of
// the seven demos shipped a sun/moon button, and in three of them the tooltip
// and chart-label colours were written as fixed dark values that ignored the
// theme — so pressing it produced unreadable text. These are instruments; the
// site's light half is where a tool becomes the client's own.
//
// T IS A MODULE CONSTANT NOW. It used to be THEMES[mode] behind a React
// context so every child could re-read it on a toggle. With one chassis there
// is nothing to re-read, so the context is gone and with it four subscribers.
// ============================================================================
const T = industrialTheme("past-due-order-triage");

/* ROLE NAMES, NOT COLOUR NAMES. These keys read `cyan`, `green`, `yellow`,
   `orange`, `red` and `slate` until 2026-08-27, which is how a palette swap
   leaves a file claiming a colour it no longer draws. The names now say what
   the colour MEANS, so the next swap cannot make them lie. */
const ACCENT = {
  control: T.accent,     // the tool's own colour — buttons, active state, links
  good:    T.good,       // done, healthy
  caution: T.sevOlive,   // needs a look, not yet late
  mid:     T.sevOrange,  // past caution
  bad:     T.bad,        // alarm
  neutral: T.textSec,    // a count that is neither good nor bad
};

/* Badge fills. The alarm badge is the one place a status is painted SOLID, and
   it is reserved for the worst step so the fill itself carries the weight. */
const FILL = { ...ACCENT };

/* Age → decay colour: five steps of the shared severity ramp. The sixth bucket
   repeats the alarm colour and is drawn FILLED instead (DECAY_FILLED below) —
   six greens-to-reds are not tellable apart on a dark ground, and anything
   past the alarm red has to go lighter to stay legible, which reads calmer
   rather than worse. A fill reads harder than any sixth hue would. */
const DECAY = {
  "0":      T.good,
  "1-7":    T.sevOlive,
  "8-30":   T.accent,
...
const DECAY_FILLED = new Set(["180+"]);

const FONT_BODY = T.fontBody;
const FONT_DATA = T.fontData;
  ...
  return (
    <Hover text={GLOSSARY[term] || term} style={{ cursor: "help" }}>
      <span style={{
        ...
        border: `1px solid ${ACCENT.control}`, color: ACCENT.control, fontSize: 9,
  ...
  return (
    <div style={{ display: "flex", width: "100%", borderRadius: 10, overflow: "hidden", border: `1px solid ${T.border}`, height: 64 }}>
      {present.map((b) => {
        ...
        /* THE LAST TWO BUCKETS SHARE THE ALARM COLOUR, so the oldest one is
           HATCHED rather than given a sixth hue — the way a board condemns a
           lane. Six greens-to-reds do not separate on a dark ground, and the
           step past the alarm red has to go lighter to stay legible, which
           reads calmer instead of worse. The hatch is the accent ink at low
           alpha, so it costs no colour the gate has not already measured. */
        const condemned = DECAY_FILLED.has(b.id);
        return (
          <Hover key={b.id} text={tip} style={{ width: w + "%", display: "block" }}>
            <div onClick={() => onPick(isActive ? "all" : b.id)} style={{
              ...
              backgroundImage: condemned
                ? `repeating-linear-gradient(45deg, transparent 0 7px, ${alpha(T.accentInk, 0.26)} 7px 14px)`
                : "none",
              display: "flex", flexDirection: "column", justifyContent: "center", paddingLeft: 11,
              borderRight: `1px solid ${T.bg}`, boxSizing: "border-box",
              outline: isActive ? `2px solid ${T.text}` : "none", outlineOffset: -2,
              boxShadow: isActive ? `0 0 0 2px ${T.bg} inset` : "none",
            }}>
              <div style={{ fontSize: 17, fontWeight: 700, color: T.accentInk, fontFamily: FONT_DATA, fontVariantNumeric: T.numeric }}>{n}</div>
              <div style={{ fontSize: 10, color: T.accentInk, opacity: 0.78, fontFamily: T.fontLabel, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: 600 }}>{b.label}</div>
            </div>
          </Hover>
        );
      })}
...
export default function PastDueOrderTriage() {
  ...
  // --- styles ---------------------------------------------------------------
  /* One warm wash off the top-right corner, the way a lamp falls on a panel.
     It was a cyan glow at 0.07; brass at 0.05 because a warm tint carries
     further on a warm chassis than a cool one did on a blue-black. */
  const page = { background: T.bg, color: T.text, minHeight: "100vh", colorScheme: "dark", accentColor: T.accent, fontFamily: FONT_BODY, fontSize: 13, fontVariantNumeric: T.numeric, backgroundImage: `radial-gradient(1200px 500px at 80% -10%, ${alpha(T.accent, 0.05)}, transparent)` };
  ...
  const btn = (primary) => ({
    ...
    border: `1px solid ${primary ? ACCENT.control : T.border}`,
    background: primary ? ACCENT.control : T.surface, color: primary ? T.accentInk : T.text,
  });
      ...
      <TipProvider>
        <div style={page}>
          <div style={wrap}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ width: 11, height: 26, borderRadius: 3, background: `linear-gradient(${ACCENT.good},${ACCENT.caution},${ACCENT.bad})`, display: "inline-block" }} />
            ...
            <div style={{ ...card, padding: "11px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", borderColor: ACCENT.control + "55" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 12, marginBottom: 16 }}>
              {[
                { k: "Orders past due", v: fmtN(result.orders.length), sub: `${fmtN(result.totalQty)} units open`, c: ACCENT.neutral },
                { k: "Close candidates", v: fmtN(result.recCounts.TECO || 0), sub: "over 90 days / superseded", c: ACCENT.bad, rec: "TECO" },
                { k: "To reschedule", v: fmtN(result.recCounts.RESCHEDULE || 0), sub: "real but late", c: ACCENT.control, rec: "RESCHEDULE" },
                { k: "Phantom score 90+", v: fmtN(Math.round(phantom90)), sub: "qty × months stale", c: ACCENT.mid, tip: GLOSSARY["Phantom score"] },
                { k: "Dispositioned", v: fmtN(dispositionedCount), sub: "in session", c: ACCENT.good },
              ].map((s) => (
                /* NO COLOURED EDGE ON EVERY TILE. It read as a status marker and
                   marked nothing: all five carried one, so its PRESENCE said
                   nothing and only its hue spoke — which the figure below
                   already says. An edge bar is kept where it appears on SOME
                   rows and not others; there it is information. Here the tile
                   earns its emphasis from the figure and a hairline rule under
                   the label, which is how a printed readout is set. */
                <div key={s.k} onClick={() => { if (s.rec) { setFRec(fRec === s.rec ? "all" : s.rec); setTab("triage"); } }}
                  style={{ ...card, padding: 13, cursor: s.rec ? "pointer" : "default" }}>
                  <Hover text={s.tip}>
                    <div style={{
                      fontSize: T.labelPx, color: T.textMuted, textTransform: "uppercase", letterSpacing: 0.6,
                      fontFamily: T.fontLabel, display: "flex", alignItems: "center",
                      paddingBottom: 5, marginBottom: 5, borderBottom: `1px solid ${T.border}`,
                    }}>{s.k}{s.tip && <Q term="Phantom score" />}</div>
                  </Hover>
                  <div style={{ fontSize: 26, fontWeight: 700, fontFamily: FONT_DATA, fontVariantNumeric: T.numeric, marginTop: 3, color: s.c }}>{s.v}</div>
            ...
            <div style={{ marginBottom: 6, display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
              ...
              <div style={{ fontSize: 11, color: T.textMuted }}>click a band to filter{fBucket !== "all" && <span style={{ color: ACCENT.control, cursor: "pointer", marginLeft: 8 }} onClick={() => setFBucket("all")}>· clear ✕</span>}</div>
            </div>
              ...
              {[["triage", "Triage"], ["materials", "Materials"], ["diff", "Snapshot diff"], ["how", "How this works"]].map(([id, label]) => {
                ...
                return (
                  <button key={id} onClick={() => setTab(id)} style={{
                    ...
                    border: `1px solid ${active ? ACCENT.control : T.border}`,
                    background: active ? ACCENT.control + "22" : T.surface, color: active ? ACCENT.control : T.textSec,
                  }}>{label}</button>
                );
              })}
                          ...
                          return (
                            <React.Fragment key={o.order}>
                              <tr onClick={() => setExpanded(isOpen ? null : o.order)} style={{ cursor: "pointer", background: isOpen ? T.surfaceAlt : rowBg, borderBottom: `1px solid ${T.border}` }}>
                                <td style={{ position: "sticky", left: 0, zIndex: 1, background: isOpen ? T.surfaceAlt : rowBg, padding: T.rowPad, borderRight: `1px solid ${T.border}` }}>
                                  <div style={{ fontFamily: FONT_DATA, fontSize: 12, color: T.text, display: "flex", alignItems: "center", gap: 6 }}>
                                    {o.order}{o.chronic && <Hover text={GLOSSARY["Chronic offender"]} style={{ cursor: "help" }}><span style={{ fontSize: 9, color: ACCENT.mid, border: `1px solid ${ACCENT.mid}`, borderRadius: 4, padding: "0 4px", fontWeight: 700 }}>CHRONIC</span></Hover>}
                                  </div>
                                ...
                                <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, color: T.textSec }}>{o.plant}</td>
                                <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, textAlign: "right" }}>{fmtN(o.qty)}<span style={{ color: T.textMuted, fontSize: 10 }}> {o.uom}</span></td>
                                <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, color: T.textSec, fontSize: 11 }}>{msToIso(o.dueMs)}</td>
                                <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, textAlign: "right", color: DECAY[o.bucket], fontWeight: 700 }}>{o.days}</td>
                                <td style={{ padding: T.rowPad, fontSize: 11, color: T.textSec }}>{/PlOrd/i.test(o.category) ? <span style={{ color: ACCENT.mid }}>Planned{o.firmed ? " · firmed" : ""}</span> : <>Production{o.firmed ? <span style={{ color: T.textMuted }}> · firmed</span> : ""}</>}</td>
                                <td style={{ padding: T.rowPad }}><Badge accent={RECS[o.rec].accent} title={RECS[o.rec].rule}>{RECS[o.rec].short}</Badge></td>
                                <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, textAlign: "right", color: T.textSec }}>{fmtN(Math.round(o.phantom))}</td>
                                <td style={{ padding: T.rowPad, fontSize: 11 }}>{!b ? "—" : b.state === "found" ? <span style={{ color: ACCENT.good }}>{b.skus.length} tool{b.skus.length === 1 ? "" : "s"}</span> : b.state === "not-found" ? <span style={{ color: T.textMuted }} title="not found in loaded scope">not in scope</span> : <span style={{ color: T.textMuted }}>no BOM</span>}</td>
                                <td style={{ padding: T.rowPad, fontSize: 11, fontFamily: FONT_DATA }}>{mv === "new" ? <span style={{ color: ACCENT.caution }}>new</span> : mv == null ? <span style={{ color: T.textMuted }}>—</span> : mv < 0 ? <span style={{ color: ACCENT.good }}>{mv} confirming</span> : <span style={{ color: ACCENT.bad }}>+{mv}</span>}</td>
                                <td style={{ padding: T.rowPad }}>
                                  <select value={disp.status || "Pending"} onClick={(e) => e.stopPropagation()} onChange={(e) => { setDisp(o.order, { status: e.target.value }); if (e.target.value !== "Pending") flash(`Disposition “${e.target.value}” saved in session — connect a backend to write it through.`); }}
                                    style={{ ...sel, padding: "3px 6px", fontSize: 11, borderColor: (disp.status && disp.status !== "Pending") ? ACCENT.good : T.border }}>
                                          ...
                                          : <div>
                                              ...
                                              {b.fkids.length > 0 && <>
                                                ...
                                                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                                                  {b.fkids.map((f) => <span key={f.mat} style={{ fontSize: 11, fontFamily: FONT_DATA, color: ACCENT.control, background: ACCENT.control + "18", borderRadius: 6, padding: "2px 7px" }}>{f.mat} · {f.desc} ×{fmtN(f.qty)}</span>)}
                                                </div>
                                              </>}
                                            </div>}
                                      </div>
                    ...
                    <tbody>
                      {result.rollup.map((r, i) => (
                        <tr key={r.plant + r.mat} style={{ background: i % 2 ? T.surface : T.bg, borderBottom: `1px solid ${T.border}` }}>
                          <td style={{ position: "sticky", left: 0, zIndex: 1, background: i % 2 ? T.surface : T.bg, padding: T.rowPad, borderRight: `1px solid ${T.border}` }}>
                            <div style={{ fontFamily: FONT_DATA, fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>{r.mat}{r.chronic && <span style={{ fontSize: 9, color: ACCENT.mid, border: `1px solid ${ACCENT.mid}`, borderRadius: 4, padding: "0 4px", fontWeight: 700 }}>CHRONIC</span>}</div>
                            <div style={{ fontSize: 11, color: T.textSec }}>{r.desc}</div>
                          </td>
                          <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, color: T.textSec }}>{r.plant}</td>
                          <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, textAlign: "right" }}>{r.orders}</td>
                          <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, textAlign: "right" }}>{fmtN(r.totalQty)}</td>
                          <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, textAlign: "right", color: r.oldestDays > 90 ? ACCENT.bad : T.textSec }}>{r.oldestDays}</td>
                          <td style={{ padding: T.rowPad, fontFamily: FONT_DATA, textAlign: "right", color: ACCENT.mid, fontWeight: 700 }}>{fmtN(Math.round(r.phantom))}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {tab === "diff" && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 14 }}>
                {[
                  { title: "Cleared since prior pull", c: ACCENT.good, rows: diff.cleared.map((p) => ({ o: p.order, m: p.mat, d: p.desc, p: p.plant, q: `${fmtN(p.qty)} gone` })), note: "Present in the prior pull, absent now — resolved or closed." },
                  { title: "New this pull", c: ACCENT.caution, rows: diff.added.map((c) => ({ o: c.order, m: c.mat, d: c.desc, p: c.plant, q: `${fmtN(c.qty)} new` })), note: "Absent from the prior pull — newly past due." },
                  { title: "Persisting & confirming", c: ACCENT.control, rows: diff.persisting.filter((x) => x.qtyDelta < 0).sort((a, b) => a.qtyDelta - b.qtyDelta).map((x) => ({ o: x.cur.order, m: x.cur.mat, d: x.cur.desc, p: x.cur.plant, q: `${x.qtyDelta} qty` })), note: "In both pulls with a quantity drop — actively confirming, leave alone." },
                ].map((col) => (
          ...
          {toast && (
            <div style={{ position: "fixed", bottom: 22, left: "50%", transform: "translateX(-50%)", zIndex: 1100, background: T.surfaceAlt, color: T.text, border: `1px solid ${ACCENT.good}`, borderRadius: 10, padding: "11px 16px", fontSize: 12.5, maxWidth: 460, boxShadow: "0 10px 32px rgba(0,0,0,0.5)" }}>
              <span style={{ color: ACCENT.good, fontWeight: 700, marginRight: 6 }}>✓</span>{toast}
            </div>
          )}
        </div>
      </TipProvider>
