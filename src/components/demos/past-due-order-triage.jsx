...
// and chart-label colors were written as fixed dark values that ignored the
...
/* ROLE NAMES, NOT COLOR NAMES. These keys read `cyan`, `green`, `yellow`,
   `orange`, `red` and `slate` until 2026-08-27, which is how a palette swap
   leaves a file claiming a color it no longer draws. The names now say what
   the color MEANS, so the next swap cannot make them lie. */
const ACCENT = {
  control: T.accent,     // the tool's own color — buttons, active state, links
...
/* Age → decay color: five steps of the shared severity ramp. The sixth bucket
   repeats the alarm color and is drawn FILLED instead (DECAY_FILLED below) —
  ...
  return (
    <div style={{ display: "flex", width: "100%", borderRadius: 10, overflow: "hidden", border: `1px solid ${T.border}`, height: 64 }}>
      {present.map((b) => {
        ...
        /* THE LAST TWO BUCKETS SHARE THE ALARM COLOR, so the oldest one is
           ...
           alpha, so it costs no color the gate has not already measured. */
          ...
          <div style={wrap}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 12, marginBottom: 16 }}>
              {[
                ...
                /* NO COLORED EDGE ON EVERY TILE. It read as a status marker and
