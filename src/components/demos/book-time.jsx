...
//
// A shop quotes labor out of a flat-rate book: a job is "2.4 hours" whatever
...
export default function BookTime({ mode = 'demo' }) {
  ...
  const r = useMemo(
    () => computeBookTime({ laborRate: state.laborRate, jobs: state.jobs }),
    [state],
  );
  ...
  return (
    <div style={pageFor('book-time')} data-demo="book-time">
      <ToolHeader toolId="book-time" house={SHOP} occasion="sample week" name="Flat Rate Efficiency Sheet" status={status} restored={restored} pack={pack} remembers="door rate and tickets">
        ...
        <div className="aiq-split" style={{ '--aiq-split': '2fr' }}>
          ...
          <div>
            <Panel title="Your door rate" note="One rate, yours. Which flat-rate book you quote from is yours too — this only reads the hours you put in it.">
              <Field label="Per hour" prefix="$" w={96} value={(state.laborRate / 100).toFixed(2)} onChange={(v) => patch((n) => { n.laborRate = toCents(v); })} />
            </Panel>
