...
import { industrialTheme } from "../kit/industrial.js";
...
/* THE INDUSTRIAL FACEPLATE, "demand & planning" band — the deepest of the
   three chassis, shared with the churn and past-due consoles.

   THE TWO-STOP GRADIENT SURVIVES; ITS TWO COLOURS DO NOT. This file paints its
   buttons, its segmented control and its active nav underline in an
   indigo-to-violet sweep. Two accent HUES is exactly the thing the brief
   rejected, but a two-STOP sweep between bright brass and deep brass reads as
   light moving across a metal face, which is the opposite of a rainbow and
   costs no second colour. So ACCENT2 becomes the dim brass, not a second hue.

   FONT_H AND FONT_B HELD THE SAME STRING UNDER TWO NAMES — a heading role that
   had been declared and never actually expressed. The band's condensed face
   now goes where the heading name always promised one. */
const T = { ...industrialTheme("order-confirmation-command-center") };
T.muted = T.textMuted;
T.grid = T.border;
T.track = T.borderStrong;
T.success = T.good;
T.warning = T.warn;
T.risk = T.bad;

const ACCENT = T.accent, ACCENT2 = T.accentDim, CYAN = T.info;
/* Five root-cause categories. Steel, olive and orange do the work two invented
   hues used to; `Other` stays deliberately quiet rather than taking a colour. */
const CAT_HUE = { Forecasting:T.accent, Logistics:T.info, Supply:T.sevOlive, Customer:T.sevOrange, Other:T.textMuted };
/* FONT_H DRAWS FIGURES, not headings, at five of its seven call sites — the
   name has meant "the big face" for as long as the file has existed. Pointing
   it at the condensed label face put Oswald on columns of percentages, and
   Oswald has no tabular digits and cannot be given any: measured 31.3px of
   drift across ten. So it takes the band's FIGURE face, and the label face
   gets its own name for the two places that really are headings. */
const FONT_H = T.fontData;
const FONT_L = T.fontLabel;
const FONT_B = T.fontBody;
...
/* THE PANEL LABEL, AND IT NOW CARRIES THE PANEL'S STATE.
   Six cards used to announce themselves with a 3-4px coloured bar down their
   left edge. Every card of a given kind had one, so the bar's presence said
   nothing and only its hue spoke — and each of those cards already had this
   label sitting inside it saying the same thing in words. The colour moves
   onto the label, where it lands on something a reader is already looking at,
   and the panel keeps one even border like a plate on a rack. */
function Eyebrow({ children, tone }){
  return <div style={{
    fontSize:11,fontWeight:700,letterSpacing:0.8,textTransform:"uppercase",
    fontFamily:T.fontLabel,color:tone||"var(--muted)",marginBottom:9,
    display:"flex",alignItems:"center",gap:7,
  }}>
    {tone && <span style={{width:7,height:7,borderRadius:1,background:tone,flex:"0 0 auto"}} />}
    {children}
  </div>;
}
    ...
    <div style={{display:"grid",gridTemplateColumns:"minmax(248px,280px) 1fr",gap:18,alignItems:"stretch"}} className="hero">
      ...
      <Card style={{display:"flex",flexDirection:"column",justifyContent:"center",gap:14}}>
        <div>
          ...
          <div style={{fontSize:19,lineHeight:1.45,color:T.text,fontFamily:FONT_B,fontWeight:400}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
          {[
            ...
            <div key={i} style={{paddingLeft:0}}>
              <div style={{fontSize:10.5,color:T.muted,textTransform:"uppercase",letterSpacing:0.6,fontFamily:T.fontLabel,paddingBottom:5,marginBottom:5,borderBottom:`1px solid ${T.border}`}}>{s.l}{s.t&&<Q term={s.t}/>}</div>
...
function ActionCard({ cause, actions, S, T }){
  return <Card>
    <Eyebrow tone={ACCENT}>Recommended actions — {cause}</Eyebrow>
        ...
        {cause && <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 200px",gap:14,alignItems:"start"}} className="actrow">
            ...
            <button onClick={()=>go("worklist",{cause})} className="lift"
              style={{cursor:"pointer",border:"none",borderRadius:12,padding:"14px 16px",color:"var(--accent-ink)",fontWeight:800,fontSize:13.5,
...
function CustomerView({ D, wk, other, S, T, go }){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    {red.length>0 && <Card>
      <Eyebrow tone={S.risk}>Service crisis — customers under 70% confirmation</Eyebrow>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>exportXlsx("filtered")} className="lift"
            style={{cursor:"pointer",border:"none",borderRadius:9,padding:"8px 13px",color:"var(--accent-ink)",fontWeight:700,fontSize:12.5,
          ...
          <tbody>
            {rows.slice(0,400).map((r,i)=>(
              <tr key={r.doc+r.it+i} style={{background:i%2?T.bg:T.surface}}>
                <td style={{position:"sticky",left:0,zIndex:1,background:i%2?T.bg:T.surface,padding:T.rowPad,color:T.text,fontWeight:600,whiteSpace:"nowrap",borderBottom:`1px solid ${T.grid}`}}>{r.cust}</td>
                <td title={r.rsn} style={{padding:T.rowPad,color:T.text,whiteSpace:"nowrap",borderBottom:`1px solid ${T.grid}`}}>{r.det}</td>
                <td style={{padding:T.rowPad,color:CAT_HUE[r.cat]||T.textSec,borderBottom:`1px solid ${T.grid}`}}>{r.cat}</td>
                <td style={{padding:T.rowPad,color:T.textSec,borderBottom:`1px solid ${T.grid}`}}>{r.prod}</td>
                <td style={{padding:T.rowPad,color:T.textSec,borderBottom:`1px solid ${T.grid}`}}>{r.mat}</td>
                <td style={{padding:T.rowPad,color:T.textSec,textAlign:"right",borderBottom:`1px solid ${T.grid}`}}>{r.doc}</td>
                <td style={{padding:T.rowPad,color:T.textSec,borderBottom:`1px solid ${T.grid}`}}>{r.site}</td>
                <td style={{padding:T.rowPad,color:T.textSec,borderBottom:`1px solid ${T.grid}`}}>{r.sup}</td>
                <td style={{padding:T.rowPad,color:T.text,fontWeight:700,textAlign:"right",borderBottom:`1px solid ${T.grid}`}}>{fmt(r.uq)}</td>
                <td style={{padding:T.rowPad,color:T.textSec,textAlign:"right",whiteSpace:"nowrap",borderBottom:`1px solid ${T.grid}`}}>{r.crdd}</td>
              </tr>))}
          </tbody>
        </table>
      </div>
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <Card>
      ...
      <button onClick={onReload} className="lift" style={{marginTop:12,cursor:"pointer",border:"none",borderRadius:9,padding:"9px 14px",
        color:"var(--accent-ink)",fontWeight:700,fontSize:12.5,background:`linear-gradient(135deg,${ACCENT},${ACCENT2})`}}>↻ Reload sample data</button>
    </Card>
    ...
    <Card>
      <Eyebrow tone={S.warning}>Honest surfaces — what the tool reconciles and what it can't compute</Eyebrow>
...
/* TABULAR FIGURES SET ONCE AT THE ROOT, because this property inherits and
   the alternative is remembering it at every figure site. Two of the three
   bands draw their numbers in a PROPORTIONAL face (Inter, Rubik), where ten
   digits drift more than 80px without it. */
.occ-root{ -webkit-font-smoothing:antialiased; text-rendering:optimizeLegibility; font-variant-numeric:tabular-nums; color-scheme:dark; accent-color:var(--accent) }
...
.seg button[data-on="1"]{ background:linear-gradient(135deg,var(--accent),var(--accent2)); color:var(--accent-ink) }
...
export default function App(){
  ...
  const vars = {
    ...
    /* Ink for text sitting ON the brass sweep. It was a literal #fff in three
       inline styles and once in the stylesheet, which is 1.9:1 on brass. */
        ...
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"13px 22px 0" }}>
          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:16, flexWrap:"wrap" }}>
            <div style={{ display:"flex", alignItems:"center", gap:13 }}>
              ...
              <div>
                <div style={{ fontFamily:FONT_L, fontSize:19, fontWeight:700, letterSpacing:0.2, lineHeight:1.1 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:13, flexWrap:"wrap" }}>
              <div style={{ textAlign:"right", lineHeight:1.15 }}>
                ...
                <div style={{ fontSize:10, color:T.muted, letterSpacing:0.6, textTransform:"uppercase", fontFamily:T.fontLabel }}>OCR {wk}</div>
              </div>
