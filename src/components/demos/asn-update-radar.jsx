...
import {
  Radar, Search, X, Filter, Download, Info, ChevronDown, ChevronRight,
    ...
    <TipCtx.Provider value={{ show, hide }}>
      ...
      {tip && (
        <div style={{
          ...
          /* THIS TOOLTIP IGNORED THE THEME — fixed dark values inside a file that
             shipped a light mode, so the tip went unreadable the moment anyone
             pressed the switch. One of three such sites across the seven. */
          background: alpha(T.surfaceAlt, 0.97), color: T.text, fontSize: 12.5, lineHeight: 1.5,
...
/* ============================================================================
   ...
   ========================================================================== */
/* THE INDUSTRIAL FACEPLATE, "inbound & inventory" band. The warmest and
   lightest of the three chassis, hairline rules, tightest rows — a receiving
   terminal, which is a queue you scan down rather than a page you read.
   Shared with the staging triage board.

   THE FIVE `*Bg` WASHES ARE DERIVED NOW, not typed. They were written as
   `rgba(255,107,107,0.13)` — the risk colour's own channels, keyed in a second
   time by hand. Move the risk colour and the wash under it stayed where it
   was, with nothing erroring. alpha() takes the colour from the token.

   ITS PALETTE CAME FROM A DIFFERENT LIBRARY TO EVERY OTHER TOOL HERE. The old
   values are Open Color — #ff6b6b, #748ffc, #51cf66 — where two of the seven
   used Tailwind's defaults and four used hand-picked slate. Three colour
   systems across seven tools sold as one practice's bench. */
const T = { ...industrialTheme("asn-update-radar") };
T.name = "industrial";
T.risk = T.bad;
T.warn = T.sevOrange;   // NOT the brass: this tool shows risk and warn side by side
T.ok = T.good;
T.info = T.info;
T.riskBg = alpha(T.bad, 0.13);
T.warnBg = alpha(T.sevOrange, 0.13);
T.okBg = alpha(T.good, 0.12);
T.infoBg = alpha(T.info, 0.13);
T.mutedBg = alpha(T.textSec, 0.10);
...
export default function ASNUpdateRadar() {
  ...
  const cell = { padding: T.rowPad, borderBottom: `1px solid ${T.border}`, fontSize: 12.5, verticalAlign: "top" };
  const th = { ...cell, position: "sticky", top: 0, zIndex: 2, background: T.surfaceAlt, color: T.textSec, fontSize: 10.5, fontWeight: 600, textTransform: "uppercase", fontFamily: T.fontLabel, letterSpacing: ".03em", cursor: "pointer", userSelect: "none", whiteSpace: "nowrap" };
  ...
  return (
    <TipProvider>
      {/* THE ONLY ONE OF THE SEVEN THAT NAMED NO FACE AT ALL. Its root asked for
          `ui-sans-serif`, which resolves to whatever the operating system feels
          like — so this tool spoke in a different voice on every machine, and
          none of them was a decision. The band's faces now come from the module,
          and the tabular setting sits here because it INHERITS: Rubik is a
          proportional face and drifts 83px across ten digits without it. */}
      <div style={{ background: T.bg, color: T.text, minHeight: "100vh", colorScheme: "dark", accentColor: T.accent, fontFamily: T.fontBody, fontVariantNumeric: T.numeric }}>
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: narrow ? "16px 12px 48px" : "20px 18px 60px" }}>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ margin: "2px 0 0", fontSize: 12.5, color: T.textMuted }}>
                  Daily inbound worklist — {NORTHPOINT.company} <span style={{ opacity: .6 }}>· demo data</span>
                </p>
              </div>
            </div>
            ...
            <div style={{ marginTop: 16, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 11, overflow: "hidden" }}>
              ...
              {showWide && (
                <div style={{ padding: "0 15px 14px", display: "grid", gridTemplateColumns: narrow ? "1fr" : "repeat(auto-fit,minmax(280px,1fr))", gap: 18 }}>
                  {/* Vessel movements */}
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 600, color: T.textMuted, textTransform: "uppercase", letterSpacing: ".06em", fontFamily: T.fontLabel, marginBottom: 7 }}>Vessel berthing shifts</div>
                  ...
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 600, color: T.textMuted, textTransform: "uppercase", letterSpacing: ".06em", fontFamily: T.fontLabel, marginBottom: 7 }}>Big container reschedules (≥{TUNING.BIG_MOVE_DAYS}d)</div>
                    ...
                    {result.movers.slice(0, TUNING.WIDE_LIST_CAP).map((m) => (
                      <Hover key={m.container} text={<span><b>{m.container}</b>{m.vessel ? ` · ${m.vessel}` : ""}<br />{m.lines} lines · {m.materials} materials · basis {m.basis}<br />{m.from} → {m.to}</span>}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "6px 0", borderBottom: `1px solid ${T.border}` }}>
                          <span style={{ fontSize: 12, fontFamily: T.fontData, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.container}</span>
                          <span style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
          ...
          <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
            <button onClick={() => exportXLSX(result.rows, "full")} title="Every row in the loaded worklist, regardless of the current filter or search"
              style={{ background: T.accent, color: T.accentInk, border: "none", borderRadius: 8, padding: "7px 13px", cursor: "pointer", fontSize: 12.5, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6 }}>
                  ...
                  return (
                    <tr key={r.asnId} style={{ opacity: d ? 0.55 : 1 }}>
                      ...
                      <td style={{ ...cell, position: "sticky", left: 0, zIndex: 1, background: rowBg, minWidth: 230 }}>
                        ...
                        <div style={{ fontSize: 11.5, color: T.textMuted, fontFamily: T.fontData }}>
                          {r.asnId} · {r.material} · qty {r.qty}
                        </div>
                        <div style={{ fontSize: 11.5, color: T.textMuted, marginTop: 1 }}>
                          {r.supplier} · {r.vessel || r.mode}{r.container ? <> · <span style={{ fontFamily: T.fontData }}>{r.container}</span></> : null}
                        </div>
                      </td>
