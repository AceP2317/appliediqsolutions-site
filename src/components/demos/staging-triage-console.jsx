...
/* THE INDUSTRIAL FACEPLATE, "inbound & inventory" band — shared with the ASN
   ...
   ITS COLOR NOW COMES THROUGH CUSTOM PROPERTIES, and the utilities stayed.
   Rewriting 142 class strings into style props would have put the layout at
   risk to change the paint, so only the COLOR utilities moved: `text-[var(--ind-text)]`
   ...
   unrelated color systems sitting in one file. */
...
const INK = {
  ...
  /* The band's row rhythm, reaching Tailwind the same way its colors do. */
  "--ind-row": T.rowPad,
...
export default function StagingTriageConsole() {
  ...
  /* THE VARIABLES LAND ON THE ROOT BELOW, so every utility in this file
     ...
     around it. The ground is the band's PAGE color, not its panel color: the
