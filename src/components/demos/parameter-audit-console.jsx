...
import { RefreshCw, Download, ArrowLeft, Info, Cpu, X } from "lucide-react";
...
/* Resolved here and not lower down: TGT below reads it at module level, and a
   const is unreachable until its own line has run. */
const T = { ...industrialTheme("parameter-audit-console") };
T.borderHi = T.borderStrong;
T.rowHover = T.surfaceAlt;
T.thBg = T.surfaceAlt;
...
const TGT = {
  /* THESE ARE TEXT, not chart fills: GeoBadge paints the tier name IN its own
     colour on a 12% wash of itself. So each one has to clear the reading floor
     against the panel, which is why the dim brass is not here — it clears 3:1
     and this needs 4.5. Two of the seven take neutrals, and honestly so: an
     in-house part is not sourced from anywhere, and an unclassified one is the
     dimmest thing on the screen by design. */
  jit:      { l: "JIT / Release", d: 0.5, c: T.accent },
  local:    { l: "Local",         d: 1.5, c: T.good },
  regional: { l: "Regional",      d: 3,   c: T.sevOlive },
  domestic: { l: "Domestic",      d: 7,   c: T.sevOrange },
  overseas: { l: "Overseas",      d: 30,  c: T.info },
...
const MO = { fontFamily: T.fontData, fontVariantNumeric: T.numeric };

/* THE INDUSTRIAL FACEPLATE, "master data & parameters" band — the cool
   chassis, square corners, heaviest rules, monospaced labels. Shared with the
   BOM explorer, because a parameter table and a bill of material are the same
   kind of screen.

   FOUR SEVERITIES, THREE COLOURS AND A GREY. `LOW` takes the dim neutral
   rather than a fourth hue: a low finding is one you are not acting on today,
   and giving it a colour makes a list of them look like an alarm board. */
const SEV = { CRITICAL: T.bad, HIGH: T.sevOrange, MEDIUM: T.sevOlive, LOW: T.textMuted };
/* Twelve verbs, five colours: raise it, ease it, do it now, look at it, tidy it. */
const DIRC = { INCREASE:T.good, PREPARE:T.good, DECREASE:T.sevOlive, REDUCE:T.sevOlive, NEGOTIATE:T.sevOlive, URGENT:T.bad, DISPOSE:T.bad, REVIEW:T.accent, CHANGE:T.accent, SET:T.accent, CLASSIFY:T.textMuted, CLEAN:T.textMuted };
...
function TipLayer({ tip }) {
  ...
  return (<div style={{ position: "fixed", left: x, top: tip.y + 14, width: w, zIndex: 9999, background: T.surfaceAlt, color: T.text, border: `1px solid ${T.borderStrong}`, borderRadius: 8, padding: "9px 11px", fontSize: 11.5, lineHeight: 1.5, boxShadow: "0 8px 24px rgba(0,0,0,.45)", pointerEvents: "none" }}><div style={{ fontWeight: 700, marginBottom: 3, color: T.accent, ...MO, fontSize: 11 }}>{tip.title}</div>{tip.body}</div>);
}
...
/* ============================ SMALL UI PRIMITIVES ========================= */
function Badge({ s }) { const c = SEV[s] || SEV.LOW; return (<span style={{ background: c, color: T.accentInk, padding: "2px 7px", borderRadius: 4, fontSize: 9, fontWeight: 700, letterSpacing: .6, ...MO }}>{s}</span>); }
...
export default function App() {
  ...
  if (selMat && data) {
    ...
    return (
      <div style={{ background: T.bg, color: T.text, minHeight: "100vh", colorScheme: "dark", accentColor: T.accent, fontFamily: T.fontBody, fontVariantNumeric: T.numeric, padding: 16 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, ...MO }}>
              ...
              <tbody>
                {paramRows.map((r, i) => {
                  ...
                  return (<tr key={i} style={{ background: ch ? T.accent + "11" : "transparent" }}><td style={{ padding: "4px 8px" }}>{r.p}</td><td style={{ padding: "4px 8px", color: ch ? SEV.MEDIUM : T.text }}>{r.cur}</td><td style={{ padding: "4px 8px", color: r.note ? SEV.MEDIUM : ch ? T.good : T.text, fontWeight: ch ? 600 : 400 }}>{r.rec}</td><td style={{ padding: "4px 8px", color: !ch ? T.textMuted : (r.d > 0 ? T.good : SEV.MEDIUM) }}>{r.note ? "constraint" : r.txt ? (ch ? "\u2192" : "OK") : ((r.d > 0 ? "+" : "") + I(r.d))}</td><td style={{ padding: "4px 8px", color: T.textMuted, fontSize: 9 }}>{r.path}</td></tr>);
                })}
              </tbody>
            </table>
          </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          <KPI T={T} label="Avg / Mo" value={N(m.am)} color={T.good} />
          <KPI T={T} label="MoS" value={m.mos > 100 ? "\u221e" : N(m.mos)} color={m.mos > 6 ? SEV.MEDIUM : m.mos < 1 ? SEV.CRITICAL : T.good} />
          <KPI T={T} label="Cur AvgInv $" value={F(m.cAvgV)} color={T.textMuted} />
          <KPI T={T} label="Rec AvgInv $" value={F(m.rAvgV)} color={T.accent} />
          <KPI T={T} label="Inv Δ $" value={F(m.invD)} sub={m.invD > 0 ? "release" : "invest"} color={m.invD > 0 ? T.good : SEV.MEDIUM} />
        </div>
  ...
  return (
    <TipProvider>
      <div style={{ background: T.bg, color: T.text, minHeight: "100vh", colorScheme: "dark", accentColor: T.accent, fontFamily: T.fontBody, fontVariantNumeric: T.numeric, padding: 14 }}>
        {/* header */}
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8, flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <button onClick={() => exportXlsx(liveAll, "full")} title="Every live finding in the dataset, ignoring filters" style={{ padding: "5px 9px", background: alpha(T.good, 0.13), border: `1px solid ${T.good}`, color: T.good, borderRadius: 5, fontSize: 10, cursor: "pointer", ...MO, display: "inline-flex", alignItems: "center", gap: 4 }}><Download size={12} />Full ({liveAll.length})</button>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
          ...
          {Object.keys(dismissed).length > 0 && <KPI T={T} label="Dismissed" value={Object.keys(dismissed).length.toLocaleString()} sub="this session" color={T.good} />}
        </div>
                ...
                <tbody>
                  {filtered.slice(0, 400).map((f) => (
                    <tr key={f.fid} onClick={() => setSelMat(f.pl + "||" + f.mat)} title={f.act} style={{ cursor: "pointer", borderBottom: "1px solid " + T.border }}
                      ...
                      <td style={{ padding: T.rowPad }}><Badge s={f.sev} /></td>
                      <td style={{ padding: T.rowPad }}><DirBadge dir={f.dir} /></td>
                      <td style={{ padding: T.rowPad, fontSize: 10, ...MO, position: "sticky", left: 0, background: T.bg, zIndex: 1 }}>{f.mat}</td>
                      <td style={{ padding: T.rowPad, color: T.textSec, fontSize: 10, maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.desc}</td>
                      <td style={{ padding: T.rowPad, color: T.textMuted, fontSize: 10, ...MO }}>{f.pl}</td>
                      <td style={{ padding: T.rowPad }}><GeoBadge geo={f.geo} /></td>
                      <td style={{ padding: T.rowPad, color: T.textMuted, fontSize: 10, ...MO }}>{f.mc}</td>
                      <td style={{ padding: T.rowPad, fontSize: 10, ...MO, textAlign: "center" }}>{f.abc !== "\u2014" ? f.abc : ""}{f.xyz !== "\u2014" ? f.xyz : ""}</td>
                      <td style={{ padding: T.rowPad, color: f.pc >= 20 ? SEV.HIGH : f.pc >= 5 ? SEV.MEDIUM : T.textMuted, fontSize: 10, ...MO, textAlign: "right" }}>{f.pc || "\u2014"}</td>
                      <td style={{ padding: T.rowPad, fontWeight: 700, fontSize: 10, ...MO, textAlign: "right", color: T.text }}>{F(f.exp)}</td>
            ...
            <div style={{ ...card, padding: 10 }}>
              ...
              <ResponsiveContainer width="100%" height={230}>
                <BarChart data={ctrlChart} margin={{ left: 0, right: 10 }}>
                  ...
                  <Bar dataKey="e" radius={[3, 3, 0, 0]} fill={T.info} fillOpacity={.85} cursor="pointer" isAnimationActive={!REDUCE_MOTION} onClick={d => { if (d?.name) { setCtrlF(d.name); setView("worklist"); } }} />
                </BarChart>
              </ResponsiveContainer>
            </div>
        ...
        {toast && <div style={{ position: "fixed", bottom: 18, left: "50%", transform: "translateX(-50%)", background: T.surfaceAlt, color: T.text, border: "1px solid " + T.accent, borderRadius: 8, padding: "9px 14px", fontSize: 11.5, ...MO, zIndex: 9999, maxWidth: 460, boxShadow: "0 8px 24px rgba(0,0,0,.4)", display: "flex", alignItems: "center", gap: 8 }}><Cpu size={14} color={T.accent} />{toast}</div>}
      </div>
    </TipProvider>
  );
}
