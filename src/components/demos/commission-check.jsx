...
export default function CommissionCheck({ mode = 'demo' }) {
  ...
  return (
    <div style={pageFor("commission-check")} data-demo="commission-check">
      <ToolHeader toolId="commission-check" house={AGENCY} occasion="sample statement" name="Commission Reconciliation" status={status} restored={restored} pack={pack} remembers="rate schedule">
                  ...
                  <tbody>
                    {r.rows.map((p, i) => (
                      <tr key={p.id} style={{ background: p.unpriced ? A.badTint : (p.variance != null && p.variance < 0 ? A.warnTint : 'transparent') }}>
                        ...
                        <td style={td}>
                          {/* THIS CAP WAS 190 AND IT WAS THE WHOLE TABLE'S SLACK, narrowed
                              2026-09-09. 190 fitted the longest sample option — "Tidewater
                              Mutual · Homeowners" — exactly, so this column dictated the
                              table's natural width and the table came out at 769.3px inside
                              a 769px panel. Zero slack, sub-pixel.

                              THAT PASSED HERE AND FAILED IN CI, deterministically, which is
                              what made it look like an environment quirk. It is not: text
                              measures a shade wider on the runner, the table asked for 773px
                              in the same 769px box, and the last column scrolled out of
                              sight. Both machines were right — one was simply on the correct
                              side of a boundary the table had no margin against.

                              A SELECT IS THE RIGHT COLUMN TO TAKE IT FROM. Its closed state
                              is a summary; the full option text is never hidden, because
                              opening the dropdown is not width-limited. Every other column
                              here is a fixed-width Cell or a computed figure, and shrinking
                              one of those would hide a number. */}
                          <select aria-label={`carrier and product for ${p.label}`} value={p.key} onChange={(e) => patch((n) => { n.items[i].key = e.target.value; })}
                            style={{ background: T.bg, color: T.text, border: `${S.rule} solid ${T.border}`, borderRadius: S.radiusSm, padding: '5px 6px', fontSize: 12, maxWidth: 164 }}>
            ...
            <Panel title="What the statement comes to" right={
              ...
              {short.length > 0 && (
                <div style={{ marginTop: 12, padding: '10px 12px', borderRadius: S.radiusSm, background: A.badTint, border: `${S.rule} solid ${A.bad}`, fontSize: 12.5, lineHeight: 1.6 }}>
                  ...
                  A total that nets to nothing can still hide an underpayment canceled out by an
