...
import {
  Search, Boxes, Network, Download, Info, RefreshCw, AlertTriangle,
...
// =============================================================================
/* THE INDUSTRIAL FACEPLATE, "master data & parameters" band. The one COOL
   chassis of the three, square-cornered, heaviest rules, and a monospaced
   label face — so a screen of part numbers reads as a printout rather than as
   a dashboard. Shared with the parameter audit console.

   TEN `*Dim` KEYS CAME OUT. Five in each mode, declared since June and
   referenced by nothing — a whole tint scale carried in the file for a
   treatment that was never built.

   THE HUE NAMES ARE GONE TOO. `cyan` is the retired identity's own color, and
   a key called `cyan` holding brass is the kind of lie a palette swap leaves
   behind: no gate sweeps for a NAME. */
const T = { ...industrialTheme("bom-explorer") };
T.control = T.accent;   // was `cyan` — the tool's own color
T.ok = T.good;          // was `green`
T.watch = T.sevOlive;   // was `yellow`
T.late = T.sevOrange;   // was `orange`
T.alarm = T.bad;        // was `red`
T.onAccent = T.accentInk;
...
function bucketStyle(theme) {
  return {
    ...
    phantom:      { label: "Phantom assembly", color: theme.watch, key: "phantom" },
    assembly:     { label: "Sub-assembly", color: theme.control, key: "assembly" },
    purchased:    { label: "Purchased", color: theme.late, key: "purchased" },
    in_house:     { label: "In-house", color: theme.ok, key: "in_house" },
    unclassified: { label: "Unclassified", color: theme.alarm, key: "unclassified" },
  };
}
      ...
      {tip && (
        <div
          style={{
            ...
            /* THE TOOLTIP IGNORED THE THEME. These three were fixed dark values
               sitting inside a file that shipped a light mode, so the tip stayed
               near-black on white the moment anyone pressed the switch. The
               switch is gone; the values now read from the chassis regardless. */
            background: alpha(T.surfaceAlt, 0.97), color: T.text, border: `1px solid ${T.borderStrong}`,
...
export default function App() {
  ...
  return (
    <TipLayer>
      <div style={{ minHeight: "100vh", colorScheme: "dark", accentColor: T.accent, background: page.bg, color: page.text, fontFamily: FONT, fontSize: 14 }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "20px 20px 56px" }}>
          ...
          <header style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 18 }}>
            <div style={{ display: "flex", gap: 13, alignItems: "center" }}>
              <div style={{
                ...
                background: `linear-gradient(135deg, ${page.control}, ${page.ok})`, color: page.onAccent, flexShrink: 0,
              }}>
                <Network size={23} />
              </div>
              <div>
          ...
          <div style={{
            ...
            fontSize: 12.5, color: page.textSec,
          }}>
            <Cpu size={15} style={{ color: page.control, flexShrink: 0 }} />
            <span>
          ...
          <div style={{ ...card, padding: 16, marginBottom: 16 }}>
            <p style={{ margin: "0 0 12px", fontSize: 13.5, color: page.textSec, lineHeight: 1.6 }}>
              ...
              <b style={{ color: page.control }}>{stats.counts.assembly}</b> are <Hover text={GLOSSARY["Assembly"]}><u style={us}>sub-assemblies</u></Hover>,{" "}
              <b style={{ color: page.ok }}>{stats.counts.in_house}</b> in-house and{" "}
              <b style={{ color: page.late }}>{stats.counts.purchased}</b> purchased components,{" "}
              <b style={{ color: page.watch }}>{stats.counts.phantom}</b> <Hover text={GLOSSARY["Phantom"]}><u style={us}>phantoms</u></Hover>, and{" "}
              <b style={{ color: page.textMuted }}>{stats.counts.document}</b> document positions.{" "}
              <b style={{ color: page.alarm }}>{stats.neg}</b> positions carry a negative quantity (by-product credit) — find them with <code style={codeChip(page)}>qty:&lt;0</code>.
            </p>
          ...
          {/* ---- Tabs ----
               flexWrap, because three tabs at 15px of side padding come to 407px
               and a 390px phone has 350 to give them. A flex row with nowhere to
               go pushes the third tab off the page, and html/body carry
               overflow-x: clip, so it is cut off rather than reachable. */}
            ...
            {[
              ...
              <button key={k} onClick={() => setTab(k)}
                style={{
                  ...
                  background: tab === k ? page.control : page.surface,
                  color: tab === k ? page.onAccent : page.textSec,
                  border: `1px solid ${tab === k ? page.control : page.border}`,
                }}>
                <Icon size={15} /> {label}
              </button>
            ))}
...
function codeChip(page) {
  return { fontFamily: MONO, fontSize: 12, background: page.surfaceAlt, border: `1px solid ${page.border}`, borderRadius: 5, padding: "1px 5px", color: page.control };
}
function btn(page, kind) {
  ...
  if (kind === "primary") return { ...base, background: page.control, color: page.onAccent, border: `1px solid ${page.control}` };
  ...
  return (
    <div style={{ background: page.surface, border: `1px solid ${page.border}`, borderRadius: 12, padding: 16, marginBottom: 16 }}>
      ...
      <Row title="Classification (the colored buckets)">
        ...
        The buckets partition the worklist — counts always sum to the total. <b style={{ color: page.alarm }}>Negative quantities</b> are a flag overlaid on whatever bucket a row sits in, not a bucket of their own.
      </Row>
...
function ResultTable({ page, B, rows, allRows }) {
  ...
  const th = (label, w, lead) => (
    <th key={label} style={{
      ...
      /* THE LABEL ROLE FOR THIS BAND. Master data letters its labels
         monospaced and uppercase over a heavy rule, so a screen of part
         numbers reads as a printout rather than as a dashboard. */
      background: page.surfaceAlt, color: page.textSec, fontWeight: 600, fontSize: 10.5,
      fontFamily: page.fontLabel, textTransform: "uppercase", letterSpacing: 0.7,
      textAlign: "left", padding: page.rowPad, borderBottom: `${page.rule}px solid ${page.borderStrong}`,
      ...
      <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12.5 }}>
        <thead>
          <tr>
            ...
            <th style={{ position: "sticky", top: 0, zIndex: 2, background: page.surfaceAlt, color: page.textSec, fontWeight: 700, fontSize: 11.5, textAlign: "left", padding: page.rowPad, borderBottom: `1px solid ${page.border}`, minWidth: 44 }}>
              <span style={{ display: "inline-flex", alignItems: "center" }}>Lvl<Q term="Level" theme={page} /></span>
            </th>
            {th("Item", 56)}{th("Component", 100)}
            <th style={{ position: "sticky", top: 0, zIndex: 2, background: page.surfaceAlt, color: page.textSec, fontWeight: 700, fontSize: 11.5, textAlign: "right", padding: page.rowPad, borderBottom: `1px solid ${page.border}`, minWidth: 78 }}>
              <span style={{ display: "inline-flex", alignItems: "center" }}>Qty / parent<Q term="Component Qty" theme={page} /></span>
            </th>
            {th("UoM", 50)}
            <th style={{ position: "sticky", top: 0, zIndex: 2, background: page.surfaceAlt, color: page.textSec, fontWeight: 700, fontSize: 11.5, textAlign: "left", padding: page.rowPad, borderBottom: `1px solid ${page.border}`, minWidth: 52 }}>
          ...
          {rows.slice(0, 600).map((r) => {
            ...
            return (
              <tr key={r.id}
                ...
                <td style={{ ...td(page), textAlign: "right", fontFamily: MONO, color: neg ? page.alarm : page.text, fontWeight: neg ? 700 : 400 }}>
                  {r.comp ? fmtQty(r.qty) : ""}
                </td>
                ...
                <td style={td(page)}>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                    ...
                    {neg && <Hover text="Negative quantity — a by-product / recovered-material credit, not an error."><AlertTriangle size={13} style={{ color: page.alarm }} /></Hover>}
                  </span>
                </td>
              </tr>
            );
    ...
    <div>
      ...
      <div style={{ background: page.surface, border: `1px solid ${page.border}`, borderRadius: 12, padding: 16, marginBottom: 14 }}>
        <div style={{ fontSize: 13.5, color: page.textSec, lineHeight: 1.6 }}>
          ...
          <b style={{ color: page.control }}>{used.length}</b> finished-good/site blocks, across{" "}
          <b style={{ color: page.ok }}>{used.reduce((n, u) => n + u.paths.length, 0)}</b> usage paths.
          ...
          <div key={u.site + u.fg} style={{ background: page.surface, border: `1px solid ${page.border}`, borderRadius: 11, padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 9, flexWrap: "wrap", gap: 8 }}>
              ...
              <div style={{ fontSize: 13, color: page.textSec }}>
                Per FG unit:{" "}
                <b style={{ fontFamily: MONO, fontSize: 16, color: Number(u.totalQty) < 0 ? page.alarm : page.ok }}>{fmtQty(u.totalQty)}</b>
              </div>
            </div>
            <div style={{ display: "grid", gap: 5 }}>
              {u.paths.map((p, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12.5, color: page.textSec, flexWrap: "wrap" }}>
                  <span style={{ fontFamily: MONO, color: page.control, fontWeight: 700, minWidth: 54 }}>×{fmtQty(p.qty)}</span>
                  {p.path.map((step, j) => (
            ...
            return (
              <div key={r.id} style={{
                ...
                <span style={{ flex: 1, color: r.comp ? page.text : page.textMuted, fontStyle: r.comp ? "normal" : "italic", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {r.compDesc}
                  {bk === "phantom" && <span style={{ marginLeft: 8, fontSize: 10.5, color: page.watch, fontWeight: 700 }}>PHANTOM</span>}
                  {bk === "assembly" && <span style={{ marginLeft: 8, fontSize: 10.5, color: page.control, fontWeight: 700 }}>ASSY</span>}
                </span>
                {r.comp && (
                  <span style={{ fontFamily: MONO, color: neg ? page.alarm : page.textSec, fontWeight: neg ? 700 : 400, minWidth: 70, textAlign: "right" }}>
                    {fmtQty(r.qty)} {r.uom}
                  </span>
                )}
              </div>
