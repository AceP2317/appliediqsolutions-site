...
export default function RecallSheet({ mode = 'demo' }) {
  ...
  return (
    <div style={pageFor("recall-sheet")} data-demo="recall-sheet">
      <ToolHeader toolId="recall-sheet" house={PRACTICE} occasion="sample list" name="Recall Due List" status={status} restored={restored} pack={pack} remembers="call list">
        ...
        <div className="aiq-split" style={{ '--aiq-split': '2fr' }}>
          <div>
            <Panel
              ...
              note="Furthest past first. The only ordering this applies comes from a date, never from a judgment about the person."
              right={<Btn small primary onClick={doExport}>{busy ? 'Building…' : 'Export the list'}</Btn>}
            >
              {/* THE SCROLL WRAPPER IS NOT DECORATION. Five columns of headings do
                  not fit the 346px a 390px phone leaves, and html/body carry
                  overflow-x: clip — so without it the last two columns are cut off
                  with no scrollbar and no way to reach them. */}
              {r.overdue.length === 0 ? (
