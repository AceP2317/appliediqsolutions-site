...
import { Layers, HelpCircle, Activity, X, FileSpreadsheet } from "lucide-react";

/* ============================================================================
   PALETTE — neutral light / dark, single restrained accent (NOT a brand color)
   ========================================================================== */
/* THE INDUSTRIAL FACEPLATE, "demand & planning" band — same chassis as the
   other two planning tools, so the three read as one bench.

   NO LIGHT MODE. This file was one of the three whose light half was already
   broken: the chart tooltip at the two RTooltip call sites hardcoded
   `color: T.text`, a dark-mode ink, so its text stayed near-white on a white
   tooltip the moment anyone pressed the switch. Dropping the switch deletes
   that defect rather than repairing it.

   THIS FILE'S EXTRA KEYS ARE ALIASES, NOT NEW COLOR. It was written against a
   twenty-key palette of its own — bg2, surface2, hover, borderSoft, tipBg,
   tipBorder — and every one of those resolves into the shared chassis below.
   Keeping the names means ~200 call sites did not have to move. */
const T = { ...industrialTheme("production-plan-churn") };
T.bg2 = T.surface;
T.surface2 = T.surfaceAlt;
T.hover = T.border;
T.borderSoft = T.border;
T.border = T.borderStrong;
T.textMute = T.textSec;
T.textFaint = T.textMuted;
T.accentSoft = alpha(T.accent, 0.16);
T.tipBg = T.surfaceAlt;
T.tipBorder = T.borderStrong;
/* The old palette's four hue names, kept as aliases so the call sites read the
   same. `teal`, `amber`, `risk` and `violet` are gone as VALUES — there is one
   warm color on this bench now, and green, red and steel carry the rest. */
T.teal = T.good;
T.amber = T.sevOrange;
T.risk = T.bad;
T.gray = T.textSec;
...
/* EIGHT CHANGE TYPES, SIX COLORS, AND THAT IS THE POINT. The old map gave all
   eight a hue of their own, which needed a violet and a mid-blue that exist
   nowhere else on the bench. Six of the shared set carry it instead, and the
   three COSMETIC types stay quiet on purpose — a renumbering is noise, and the
   legend spells out which is which where the color no longer does.

   `added` USED TO BE TWO COLORS: red as a type and the accent as a group, in
   the same tool, in the same view. Now one. */
const TYPE_COLOR = (T) => ({
  pullin: T.accent, pushout: T.info, added: T.risk, canceled: T.teal,
  qty: T.amber, renumber: T.textFaint, line: T.sevOlive, version: T.textFaint,
});
const GROUP_COLOR = (T) => ({ reschedule: T.gray, added: T.risk, canceled: T.teal, qty: T.amber, cosmetic: T.textFaint });
    ...
    <TipCtx.Provider value={{ show, move, hide }}>
      ...
      {tip && (
        <div style={{
          ...
          background: "var(--tip-bg)", color: T.text, border: "1px solid var(--tip-border)",
...
function Tile({ T, label, value, sub, tipText, color }) {
  ...
  return (
    <div
      ...
      {/* THE LABEL ROLE, which this tool had no expression of. The demand band
          letters its labels condensed and uppercase over a hairline, the way a
          schedule board is set — the same treatment the other two planning
          tools use, so the three read as one bench rather than three tools.
          Its figure face is Inter here, which is NOT tabular by default: the
          numeric setting below is what stops a column of counts drifting. */}
      <div style={{
        fontSize: T.labelPx, color: T.textMuted, fontWeight: 600, letterSpacing: 0.6,
        fontFamily: T.fontLabel, textTransform: "uppercase",
        paddingBottom: 5, marginBottom: 5, borderBottom: `1px solid ${T.borderSoft}`,
      }}>{label}</div>
      <div style={{ fontSize: 23, fontWeight: 700, color: color || T.text, fontFamily: MONO, fontVariantNumeric: T.numeric, lineHeight: 1 }}>{value}</div>
...
export default function App() {
  ...
  return (
    <TipLayer>
      <div style={{ ...cssVars, fontFamily: FONT, background: T.bg, color: T.text, minHeight: "100vh", colorScheme: "dark", accentColor: T.accent, paddingBottom: 48, fontVariantNumeric: T.numeric }}>
        {/* ---- HEADER ---- */}
        <div style={{ background: T.bg2, borderBottom: `1px solid ${T.border}`, padding: "14px 22px", position: "sticky", top: 0, zIndex: 50 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ marginRight: "auto" }}>
              ...
              <div style={{ fontSize: 12, color: T.textMute }}>How nervous is the plan — and who moved it · {NORTHPOINT.company} <span style={{ color: T.textFaint }}>Built by Ian Provencher</span></div>
            </div>
              ...
              {["units", "lines"].map((b) => (
                <button key={b} onClick={() => setBasis(b)} title={GLOSSARY["Basis"]}
                  style={{ padding: "6px 11px", fontSize: 12, fontWeight: 600, border: "none", cursor: "pointer",
                    background: basis === b ? T.accent : "transparent", color: basis === b ? T.accentInk : T.textMute }}>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: narrow ? "14px 12px" : "20px 22px" }}>
          ...
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 16 }}>
            <div style={{ fontSize: 13.5, lineHeight: 1.65, color: T.text }}>
              ...
              {" "}(<Q term="%-disturbed">{pct(sc.pctDisturbed)} disturbed</Q>) — {sc.byType.pullin.count} pulled in, {sc.byType.pushout.count} pushed out, {sc.byType.added.count} added, {sc.byType.canceled.count} canceled.
            ...
            {howOpen && (
              <div style={{ marginTop: 11, fontSize: 12.5, lineHeight: 1.7, color: T.textMute, borderTop: `1px solid ${T.borderSoft}`, paddingTop: 11 }}>
                ...
                <b style={{ color: T.text }}>Honest gaps.</b> Added and canceled orders are left <i>unattributed</i> — one field can't prove intent. Line figures are independent ratios, not a partition of the plant figure (see Lines &amp; Platforms).
              </div>
            )}
          </div>
          ...
          <div style={{ display: narrow ? "grid" : "flex", gridTemplateColumns: narrow ? "repeat(auto-fit, minmax(130px, 1fr))" : undefined, gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
            ...
            <Tile T={T} label="RESCHEDULE" value={fmt(basis === "units" ? sc.byType.pullin.units + sc.byType.pushout.units : sc.reschedule)}
              sub={`${sc.byType.pullin.count} in · ${sc.byType.pushout.count} out`} color={T.gray}
              tipText="Orders whose scheduled week moved (in or out). The single largest source of supplier whiplash." />
            <Tile T={T} label="ADDED / CANCELED" value={`${sc.byType.added.count} / ${sc.byType.canceled.count}`} color={T.risk}
              ...
              <div style={{ height: 230 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={trendData} margin={{ top: 8, right: 8, bottom: 4, left: -8 }}>
                    ...
                    <RTooltip contentStyle={{ background: T.tipBg, border: `1px solid ${T.tipBorder}`, borderRadius: 8, fontSize: 12, color: T.text }} />
          ...
          <Panel T={T} title="Lines & platforms" subtitle="independent per-line load, %-disturbed, and platform mix (latest snapshot)">
            <div style={{ display: "grid", gridTemplateColumns: narrow ? "1fr" : "1fr 1fr", gap: 12 }}>
              {linePanel.map((lp) => {
                ...
                const seg = [
                  { k: "B native", v: lp.comp.b, c: T.gray },
                ...
                <table style={{ borderCollapse: "collapse", fontSize: 12, minWidth: 420 }}>
                  ...
                  <tbody>
                    {matMaterials.map((mat) => (
                      <tr key={mat}>
                        <td style={{ position: "sticky", left: 0, zIndex: 1, background: T.surface, color: T.text, fontFamily: MONO, fontWeight: 600, padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}` }}>{mat}</td>
                        {matWeeks.map((w) => {
                          ...
                          return (
                            <td key={w}
                              ...
                              style={{
                                textAlign: "center", padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}`,
            ...
            right={
              <div style={{ display: "flex", gap: 7 }}>
                <button onClick={() => exportRows(scopedChanges, "full")} title="Every disturbed order in the loaded pair/scope, ignoring the on-screen filter"
                  style={{ display: "inline-flex", alignItems: "center", gap: 5, background: T.accent, color: T.accentInk, border: "none", borderRadius: 7, padding: "6px 10px", fontSize: 11.5, fontWeight: 600, cursor: "pointer" }}>
                ...
                <tbody>
                  {detailRows.map((r) => (
                    <tr key={r.id} onClick={() => setDrill({ kind: "row", rows: [r] })}
                      ...
                      <td style={{ position: "sticky", left: 0, zIndex: 1, background: drill && drill.kind === "row" && drill.rows[0].id === r.id ? T.hover : T.surface, fontFamily: MONO, padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}`, fontWeight: 600 }}>{r.po}</td>
                      <td style={{ fontFamily: MONO, padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}` }}>{r.material}</td>
                      <td style={{ padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}`, color: T.textMute, whiteSpace: "nowrap" }}>{r.line}</td>
                      <td style={{ padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}` }}>
                      ...
                      <td style={{ padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}`, color: T.textFaint, fontSize: 11, whiteSpace: "nowrap" }}>{r.pvCode}</td>
                      <td style={{ padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}`, fontFamily: MONO, color: T.textMute }}>{weekLabel(r.week)}</td>
                      <td style={{ padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}`, whiteSpace: "nowrap" }}>
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
                          <span style={{ width: 8, height: 8, borderRadius: 2, background: tColor[r.type] }} />
                          {TYPES[r.type].label}
                        </span>
                      </td>
                      <td style={{ padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}`, fontFamily: MONO, textAlign: "right" }}>{fmt(r.units)}</td>
                      <td style={{ padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}` }}>
                      ...
                      <td style={{ padding: T.rowPad, borderBottom: `1px solid ${T.borderSoft}`, color: T.textMute, maxWidth: 300, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={r.reason}>{r.reason}</td>
                    </tr>
                  ))}
                  {detailRows.length === 0 && (
        ...
        {drill && (
          <div style={{ position: "fixed", top: 0, right: 0, height: "100vh", width: 360, maxWidth: "92vw", background: T.bg2, borderLeft: `1px solid ${T.border}`, zIndex: 200, boxShadow: "-12px 0 32px rgba(0,0,0,0.35)", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ overflow: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
              {drill.rows.map((r) => (
                <div key={r.id} style={{ background: T.surface, border: `1px solid ${T.borderSoft}`, borderRadius: 10, padding: "11px 13px" }}>
