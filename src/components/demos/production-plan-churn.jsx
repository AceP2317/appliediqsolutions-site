...
/* THE INDUSTRIAL FACEPLATE, "demand & planning" band — same chassis as the
   ...
   THIS FILE'S EXTRA KEYS ARE ALIASES, NOT NEW COLOUR. It was written against a
...
/* The old palette's four hue names, kept as aliases so the call sites read the
   ...
   warm colour on this bench now, and green, red and steel carry the rest. */
T.teal = T.good;
T.amber = T.sevOrange;
T.risk = T.bad;
T.grey = T.textSec;
...
/* EIGHT CHANGE TYPES, SIX COLOURS, AND THAT IS THE POINT. The old map gave all
   ...
   legend spells out which is which where the colour no longer does.

   `added` USED TO BE TWO COLOURS: red as a type and the accent as a group, in
   the same tool, in the same view. Now one. */
const TYPE_COLOR = (T) => ({
  pullin: T.accent, pushout: T.info, added: T.risk, cancelled: T.teal,
  qty: T.amber, renumber: T.textFaint, line: T.sevOlive, version: T.textFaint,
});
const GROUP_COLOR = (T) => ({ reschedule: T.grey, added: T.risk, cancelled: T.teal, qty: T.amber, cosmetic: T.textFaint });
