...
import { tradesProse, tradeCountWord, streetFrontCountWord } from '../lib/trades.js';
...
> Custom software for small local business, built by someone who ran the businesses first. Ian Provencher spent five and a half years running a restaurant floor at a casino, then a bank's consumer lending operations, then cash management at a second bank, a clinical practice's scheduling and records, a licensed insurance practice, and now the planning floor of an appliance plant. ${streetFrontCountWord({ capital: true })} of those ${tradeCountWord()} sit on one street in New Bern, NC. He builds the software every one of them needed, and you own it outright. ${startingPrice.line} ${startingPrice.qualifier} ${shelfCount} narrow tools run right in the browser on this site, on invented data, with no login and nothing sent anywhere.
...
## For manufacturers — factory tools built to order
These are sold to manufacturers at ${origin}/supply-chain/, built to order for one plant on its own extract or run once for the written answer, and quoted per plant. Each is openable as a demo on invented data. They are NOT what a small business is being sold; each is a larger version of a problem a small business also has.
${atScale}
...
## Pages
- [Home](${origin}/): The pitch, as three doors — a website you own, Main Street for a local business, and the factory tools built to order for a manufacturer.
...
- [AI Fit Assessment](${origin}/ai-fit/): A paid assessment for a business that wants to use AI and cannot see where it fits. A read of how the work is actually done, on video with the screen shared, and a written list of what is worth building with a price beside each line. ${assessmentPrice.video.line} ${assessmentPrice.video.qualifier} It is deliberately NOT a strategy deck, a maturity score, a roadmap or a vendor shortlist, and it does not assume the answer is AI.
- [Bio](${origin}/bio/): The trades, run from the inside before a line of code.
