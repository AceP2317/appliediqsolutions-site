...
/** Days a meeting link stays usable before it stops working.
 *
 *  MOVED HERE 2026-09-13, AND IT WAS A TERM OF USE WITH NO CONSTANT UNDER IT.
 *  /terms/ told a visitor "A meeting link I send is for you and expires after two weeks", and the
 *  real expiry was `ROOM_TTL_DAYS` inside worker/index.js — a non-exported const nothing in src/
 *  could see. Two statements of one fact sharing no literal, in the one document a visitor is told
 *  they accept. Change the worker and the site's terms of use become false, silently.
 *
 *  THE WORKER STILL HOLDS ITS OWN COPY, deliberately: it is bundled separately and nothing in it
 *  imports from src/ today, so wiring that up is a build change rather than a copy fix. Instead
 *  check-dist.mjs asserts the two agree, which is the same inversion this file's header describes
 *  for the signed documents — the source of truth is here and the other copy is checked against it. */
export const MEETING_LINK_TTL_DAYS = 14;
...
/** Hours from becoming aware of unauthorized access to or loss of client data within which
