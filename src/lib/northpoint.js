 ...
 * rather than a fact about this website. The values are the `northpoint`
 * palette in `src/components/kit/industrial.js`, read through
 * `northpointTokens(slug)`. That file does NOT hand this world back, so a tool
 * imports both: its paint from there, whose plant it runs in from here.
 *
 * Three band grounds, one per group the /work/ gallery already shows, in a dark
 * and a light mode, brass for chrome and amber for warnings. A new demo built
 * for Northpoint inherits BOTH halves; see DECISIONS D120 (which amended D42),
 * `docs/factory-house-move.md`, and the `portfolio-demo` skill.
 *
 * ── WHAT THIS IS NOT ────────────────────────────────────────────────────────
 *
 ...
 *
 * If the colors lived in this file too, then `src/lib/` — which the repo keeps
...
export const HOUSE_STYLE = {
  ...
  summary: 'three band grounds, one per /work/ gallery group, in a dark mode and a light one; brass for chrome, amber for warnings, IBM Plex type; the house frame from src/components/kit/house.jsx',
  decision: 'D42, amended by D120',
};
