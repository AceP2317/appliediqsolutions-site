/* ============================================================================
   ...
   THE HONESTY CONTRACT. The labor rate, which book the shop quotes from, and
...
/**
 * @param {object} input
 * @param {number} input.laborRate  the shop's own door rate, cents per hour
...
export function computeBookTime(input) {
  ...
  const money = computeRateCard({
    ...
    rates: [{ id: 'labor', key: 'Labor', basis: 'per-unit', perUnit: laborRate, minimum: null }],
  ...
  totals.hoursLostAtRate = Math.round((totals.hoursLost * laborRate) / 100);
