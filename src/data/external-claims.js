...
export const CLAIMS = [
  ...
  {
    id: 'visibility-two-rivals-unexplained', kind: 'human',
    maxAgeDays: 90, // a competitor's marketing page, which they can rewrite without telling anyone
    where: 'src/pages/visibility.astro, the method band -- and src/lib/pricing.js, where the figure is argued from it',
    claim: 'Identity Creative Group and Lost Highway Media both publish that they get businesses into ChatGPT, Perplexity and Google AI Overviews, and neither publishes the method',
    verified: '2026-09-12',
    source: 'Read once by hand during the competitive sweep behind D113; written up under "Competitors, read once by hand on 2026-09-12 and recorded as facts a person checked" in ~/.claude/notes/appliediqsolutions.md. Not a fetchable URL from here, which is what makes this `human`.',
    note: 'REGISTERED 2026-09-13, AND THE DEFECT WAS THAT IT NEVER WAS. /visibility/ has been publishing "Two firms near here sell something like this and neither says how they do it" to visitors, and src/lib/pricing.js argues the figure from it in as many words -- "WHAT THE NUMBER RESTS ON. Two firms in Eastern North Carolina already claim this." Five copies across the repo, four of them comments, no constant under any of them and no entry here. CLAUDE.md is explicit that a claim about a system this repo does not control is a measurement and lives in this file; this one was measured properly, on a real sweep, with both firms named and a date -- and then the measurement stayed in a research note outside the repo while only the conclusion shipped. THE CLAIM IS NOT IN DOUBT. What was missing is the thing this file exists for: something that ages it and says so. NEITHER FIRM IS NAMED ON THE SITE and neither should be -- the page makes a claim about a category, not about two businesses, and naming them would turn a differentiator into a comparison nobody asked for. The names live here because a claim you cannot re-check is not a claim. NOTHING AUTOMATED EVER FETCHES EITHER HOST: they are not on REACHABLE_HOSTS and must not be added, under the 2026-08-28 rule that nothing in this repo on a schedule contacts a neighbor or a competitor. Re-checking this means the operator opening two pages himself.',
  },
];
