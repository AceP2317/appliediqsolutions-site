...
export const CLAIMS = [
  ...
  // run instead of quietly resuming the requests.
  {
    ...
    kind: 'human',
    where: 'PICKUP.md · docs/founding-builds.md',
  ...
  },
  {
    ...
    kind: 'human',
    where: 'PICKUP.md item 4 · docs/founding-builds.md',
  ...
  // ─── HUMAN — nobody can automate these, and pretending otherwise is the bug ─
  {
    ...
    where: 'docs/founding-builds.md · PICKUP.md · docs/off-repo-copy.md',
    ...
    verified: '2026-08-25',
    note: 'Restated in 15 places across 3 files and only ONE names a source. Canonical home should be docs/founding-builds.md; everything else should cite it.',
  },
  {
  ...
  },
  {
    ...
    where: 'docs/founding-builds.md · PICKUP.md item 1',
     ...
     would be worth less than the read date. */
  {
    ...
    maxAgeDays: 90, // re-read 2026-09-11 for the payments sheet and moved onto the platform clock
    where: 'src/lib/payments.js · docs/engagement/client-documents/invoice.md · src/data/getting-paid.js stripe card',
    ...
    note: 'The one that decides a design rather than a sentence is the ACH cap. At $5 flat, a $4,500 build billed in two halves costs about $14 to collect against about $135 on card — which is why the invoice document asks for a transfer on anything sizeable and the assessment can reasonably be a card link. If the cap ever moves, re-read that recommendation and not only the number. Kind moved from human to assisted on 2026-09-11 — a signed-in browser is not needed for this page, so Claude can refresh it. RE-READ 2026-09-20 IN A BROWSER when the payments sheet was restored: every figure above was still current. ONE THING IS NEW AND IS NOT IN THE CLAIM: Stripe now charges a Smart Disputes fee of 30% of the disputed amount on each dispute it wins for you, with no fee on a lost one. It is left out because the sheet prices ordinary acceptance rather than an optional dispute service, and a percentage of a disputed amount does not belong in a column of per-transaction rates.',
  },

  /* ── THE PAYMENTS AUDIT, RE-READ 2026-09-20 ───────────────────────────────
     TWELVE COMPANIES, FOURTEEN CLAIMS, AND THE WHOLE BLOCK WAS ABSENT FOR A DAY.
     D118 deleted these on 2026-09-20 with the payments sheet. The operator
     restored the sheet on 2026-09-20, and nothing here was recovered from git —
     every figure below was read again, in a browser, on that date. Re-filing a
     deleted claim from its own text would have carried the read date forward
     without the read, which is the one thing the register exists to prevent.

     THIRTEEN OF THE FOURTEEN CAME BACK UNCHANGED. The fourteenth did not, and
     the entry that replaced it is `intuit-comparison-square-only` below. Read
     that one before repeating anything about what a vendor's comparison gets
     wrong, because the comparison in question was corrected three days before
     this read.

     THE REASON FOUR OF THE NINE CARDS PRINT NO NUMBER IS IN HERE, not in the
     page copy: Toast publishes no rate, Zelle publishes no fee of its own, and
     two more refuse for reasons their own entries state. A later pass that fills
     one of those in with a plausible figure would look like tidying up and is
     the worst thing that could happen to that sheet.

     THEY CANNOT BE PROBED. No payment host is on REACHABLE_HOSTS and D64 already
     refused adding one. These are `assisted`: Claude refreshes them by driving a
     browser, nothing here reaches a vendor on a schedule, and none of them ever
     passes a check — they only age. Two of the pages below hide their fee table
     behind a click, so a scripted fetch would read the page and find no rates at
     all, then report exactly like a page whose rates had been withdrawn. */
  {
    id: 'stripe-managed-payments-mor', kind: 'assisted',
    maxAgeDays: 90,
    where: 'src/data/getting-paid.js merchant-of-record card',
    claim: 'Stripe sells a merchant-of-record product, Managed Payments, at 3.5% per successful transaction in addition to ordinary Payments fees, covering indirect tax compliance and remittance in more than 75 countries plus fraud prevention, dispute management, invoicing and customer support globally',
    verified: '2026-09-20',
    source: 'https://stripe.com/pricing',
  ...
  },
  {
    ...
    claim: 'Square US processing by plan (Free / Plus / Premium): tap, dip or swipe 2.6% + 15c, 2.5% + 15c, 2.4% + 15c; online 3.3% + 30c, 2.9% + 30c, 2.9% + 30c; online API 2.9% + 30c on all three; ACH by invoice 1% with a $1 minimum and NO fee cap on Free against a $10 cap on Plus and Premium; ACH by API 1%, $1 minimum, $5 cap on all three; manual entry or card on file 3.5% + 15c on all three; Afterpay 6% + 30c; cash or check free; EBT 1.8% + 5c; Bitcoin 0% until 2027; next-day transfers included on every plan',
    verified: '2026-09-20',
    source: 'https://squareup.com/us/en/pricing (fees table, behind the "Fees" accordion at the foot of the plan comparison)',
    note: 'READ IN A BROWSER, because the fee table is collapsed behind a click and does not appear in the page text until it is opened. THE LINE WORTH THE WHOLE CLAIM IS THE ACH CAP: on the free plan an invoice paid by bank transfer is 1% with no ceiling, so a $10,000 invoice costs $100 where Plus caps it at $10. Nothing on the plan card says so, and it is the opposite of the usual free-tier trade — here the free plan is uncapped and the paid one protects you. NEW ON 2026-09-20 AND NOT YET ON THE SHEET: a fourth tier, Square Pro, offering custom pricing and hardware discounts to businesses processing over $250,000 a year. The three-plan table above is unaffected; the card should stop saying Square has three plans if it ever did.',
  },
  {
    id: 'paypal-merchant-fees-us', kind: 'assisted',
    maxAgeDays: 90,
    where: 'src/data/getting-paid.js paypal card',
    claim: 'PayPal US merchant rates: PayPal Checkout and Guest Checkout 3.49% + $0.49; standard credit and debit 2.99% + $0.49; QR code 2.29% + $0.09; Pay with Venmo 3.49% + $0.49; Send/Receive Money for Goods and Services 2.99%; Pay Later 4.99% + fixed; international commercial transactions add 1.50%; invoicing paid by bank 1% capped at $10; PayPal Point of Sale card-present 2.29% + $0.09 and manual entry 3.49% + $0.09; chargeback $20; standard dispute $15 and high-volume dispute $30; currency conversion spread 3.00%, or 4.00% on some transaction types',
    verified: '2026-09-20',
  ...
  },
  {
    ...
    maxAgeDays: 90,
    where: 'src/data/getting-paid.js p2p-apps card',
    claim: 'Zelle publishes no fee of its own for small business use and states that eligibility runs through the bank — "Contact your bank or credit union to see if they offer Zelle to eligible small businesses"; its own safety guidance is headed "Treat Zelle Like Cash" and says to only send money to those you know and trust',
    verified: '2026-09-20',
    source: 'https://www.zelle.com/business · https://www.zelle.com/digital-payment-education',
    note: 'ZELLE IS NOT A PROCESSOR AND THE CARD MUST NOT CALL IT ONE. It moves money between bank accounts, the bank decides whether a business may use it, and there is no card network behind it — so there is no chargeback, no dispute process and no seller protection. "Treat Zelle Like Cash" is a live heading on Zelle’s own education page and is quoted rather than characterized. The no-fee half is a NEGATIVE reading: the business page carries no fee figure anywhere, which is not the same as a published promise of zero, and the card must say the bank decides.',
  },
  {
  ...
  },
  {
    ...
    claim: 'Toast publishes no card processing rate on its pricing page — the plans read "Custom pricing" and the payment-processing row reads "Simple, flat rate" with no figure anywhere on the page, software starts at $0/month for a Starter Kit and $69/month for Point of Sale, and Toast states it "is not a payment card processor, but it is a payment facilitator that partners with processors to offer card processing services"',
    verified: '2026-09-20',
    source: 'https://pos.toasttab.com/pricing (page stamped "This page was last updated on: August 20, 2026")',
    note: 'A NEGATIVE FINDING, AND IT IS LOAD-BEARING RATHER THAN A GAP IN THE READING. It is the reason payment-cost.js refuses to price the terminals card at all, and the reason that refusal prints a sentence instead of a blank. If Toast ever publishes a rate this claim goes false in the useful direction and the card gains a number — re-read the page rather than softening the sentence. The whole page text was searched for a percentage on 2026-09-20 and there was none.',
  },
  {
    id: 'quickbooks-payment-rates', kind: 'assisted',
    maxAgeDays: 90,
    where: 'src/data/getting-paid.js bundled card',
    claim: 'QuickBooks Payments US: cards and digital wallets on invoices, recurring payments or quick requests 2.99%; ACH bank payments 1%; in-person 2.5%; keyed-in cards 3.5%; buy now pay later 2.99%; instant deposits 1.75%; dispute protection from 0.99%; no setup costs or hidden fees, and merchants processing over $2,500 a month are invited to call about up to 25% off transaction costs',
    verified: '2026-09-20',
    source: 'https://quickbooks.intuit.com/payments/payment-rates/',
    note: 'Intuit’s own rates are taken as read because they are its own prices. The competitor column beside them is a separate claim and is NOT covered by this one — see intuit-comparison-square-only. quickbooks.intuit.com/payments/pricing/ redirects to a plans page with no rates on it, and the rate table sits behind this URL instead.',
  },
  {
    id: 'intuit-comparison-square-only', kind: 'assisted',
    maxAgeDays: 90,
    where: 'src/data/getting-paid.js header · the payments sheet’s framing of vendor comparisons',
    claim: 'Intuit’s payment-rates page is titled "QuickBooks Payments Processing Fees vs. Stripe & Square" and its comparison table now shows one competitor column, Square; the word Stripe appears exactly once on the page, in a footnote reading "Stripe may charge an additional 0.4%/0.5% fee per paid invoice based on plan"; the table is stamped "Rates are accurate as of 09/18/2026" and its Square figures agree with Square’s own pricing page read the same day',
    verified: '2026-09-20',
    source: 'https://quickbooks.intuit.com/payments/payment-rates/ · https://squareup.com/us/en/pricing',
    note: 'THIS REPLACES `quickbooks-competitor-table-stale`, WHICH WENT FALSE AND IS NOT COMING BACK. That claim said Intuit’s table was stamped 30 April 2026 and carried two wrong competitor figures — Stripe ACH at 1.2% against Stripe’s published 0.8% capped at $5, and Square in person at 2.6% + 10c against Square’s published 2.6% + 15c. Re-read on 2026-09-20: Intuit refreshed the table on 09/18, dropped the Stripe column entirely, and now prints 2.6% + 15c for Square, which matches. BOTH ERRORS ARE CORRECTED AND THE CLAIM IS RETIRED RATHER THAN RESTATED. The old claim’s own note said to re-read all three pages before repeating it, never just the Intuit page; doing exactly that is what killed it, and that is the register working rather than failing. WHAT SURVIVES IS WEAKER AND MUST BE STATED AS SUCH: a vendor’s own comparison is refreshed when it suits the vendor, and this one now omits the competitor whose ACH rate — 0.8% capped at $5 — beats QuickBooks’ uncapped 1%. Do NOT assert that the omission is deliberate. The counting method was a DOM read of the whole page text rather than a look at the rendered table, because the table is wider than the window and a screenshot cannot show what is not scrolled into view.',
  },
  {
    id: 'wix-payments-fees-us', kind: 'assisted',
    maxAgeDays: 90,
    where: 'src/data/getting-paid.js bundled card',
    claim: 'Wix Payments US service fees: credit and debit cards excluding American Express 2.9% + $0.30, the same for Apple Pay and Google Pay; American Express including through those wallets 3.7% + $0.30; PayPal 3.49% + $0.49; Pay Later 4.99% + $0.49; Venmo 3.49% + $0.49; point of sale 2.6% + $0; Tap to Pay 2.6% + $0.20; manual card entry 3.5% + $0.30 and American Express manual entry 4.3% + $0.30; cross-border 1.5% and currency conversion 1% on top; chargeback $15, or $20 on a PayPal payment',
    verified: '2026-09-20',
    source: 'https://support.wix.com/en/article/wix-payments-service-fees',
    note: 'THE AMERICAN EXPRESS LINE IS THE ONE WORTH CARRYING. At 3.7% against 2.9% it is nearly a point dearer on a card plenty of customers hand over without thinking, and no other option on the sheet publishes an Amex surcharge — the manual-entry Amex line at 4.3% is dearer still. wix.com/payments/pricing is a 404; the fees live in the support article and are selected by country on the page. The 2026-09-11 reading of this claim omitted Venmo’s fixed fee and said only "Venmo 3.49%"; the page reads 3.49% + $0.49 and has been corrected here.',
  },
  {
    id: 'squarespace-payment-rates', kind: 'assisted',
    maxAgeDays: 90,
    where: 'src/data/getting-paid.js bundled card',
    claim: 'Squarespace Payments US rates move with the website plan: standard cards 2.9% + $0.30 on Basic and Core, 2.7% + $0.30 on Plus, 2.5% + $0.30 on Advanced; international cards add 1.5% and currency conversion adds 1% on every plan; ACH Direct Debit 1.5% on Basic and 1% on Core, Plus and Advanced, with a $10 maximum fee printed against Advanced; premium cards 3.2% + $0.30; Klarna 5.45% + $0.30; Afterpay 6% + $0.30',
    verified: '2026-09-20',
    source: 'https://support.squarespace.com/hc/en-us/articles/27853679334157-Transaction-fees-and-payment-processing-rates (page stamped "Last updated August 3, 2026")',
    note: 'THIS IS THE CLAIM THAT PROVES THE BUNDLED CARD’S THESIS. The same sale costs different money depending on which website plan you bought, so the payment rate was decided by a website decision nobody framed as one. The premium-card line at 3.2% is the second half: a customer paying with a rewards card costs more and nothing on the plan page says so. ONE THING IS NOT SETTLED AND THE CLAIM SAYS SO RATHER THAN GUESSING: the "$10 maximum fee" sits inside the Advanced cell of the ACH row, and the table does not state whether it also applies to Core and Plus. Read it off an account before telling a client the cap covers their plan.',
  },
  {
    id: 'paddle-pricing', kind: 'assisted',
    maxAgeDays: 90,
    where: 'src/data/getting-paid.js merchant-of-record card',
    claim: 'Paddle charges 5% + 50c per checkout transaction under a headline of "No added costs, no hidden fees", covering payments, billing, cross-border sales tax compliance and protection against fraud and chargebacks; its own footnote reads "If you’re selling products under $10 or require invoicing contact us for custom pricing"',
    verified: '2026-09-20',
    source: 'https://www.paddle.com/pricing',
    note: 'Identical headline to Lemon Squeezy, which is why the card states one rate for the category rather than two. The under-$10 exclusion matters for anyone selling a cheap digital item and is the first thing to check before recommending it. The 2026-09-11 wording said "no monthly, migration or hidden fees"; the page now says "No added costs, no hidden fees" and the claim quotes what is there today.',
  },
  {
    id: 'lemonsqueezy-pricing', kind: 'assisted',
    maxAgeDays: 90,
    where: 'src/data/getting-paid.js merchant-of-record card',
    claim: 'Lemon Squeezy charges 5% + 50c per transaction for ecommerce and states "There are no monthly charges for ecommerce features, and we only take a small cut of each sale", including global payment acceptance, automated sales tax compliance and fraud protection; its own footnote reads "Some payments may be subject to additional fees"',
    verified: '2026-09-20',
  ...
  },
  {
    ...
    where: 'the founding store scope, docs/engagement/agreements/founding-store-scope.md, which promises a page that reads a client catalog on the platforms that allow it; the public store comparison that first cited this came down on 2026-09-20 (D118)',
...
/* ============================================================================
   DOES A NAMED CLAIM ID ACTUALLY EXIST HERE? Added 2026-09-20, and it closes the
   hole that this whole file exists to close.

   THE FAILURE, MEASURED RATHER THAN IMAGINED. Every comparison sheet's gate
   checked that a card's `claimIds` array was NON-EMPTY and stopped there. It
   never asked whether those ids resolve. On 2026-09-20 D118 deleted fifteen
   payment fee claims from this file and left src/data/getting-paid.js naming
   them; when that sheet was restored on 2026-09-20 its gate reported "every
   option publishes a registered claim" while FOURTEEN of the eighteen ids it
   named pointed at nothing. A card citing a deleted claim and a card citing a
   live one were the same green tick.

   WHY THAT IS THE WORST POSSIBLE SHAPE OF THIS BUG. A card with no claimIds at
   all is caught, loudly, by the existing check. A card naming a claim that has
   been deleted is worse in every way and was caught by nothing: the figure is
   published, the sheet prints a read date beside it, the gate agrees it is
   sourced, and NOTHING AGES IT \u2014 so the one mechanism that would have warned
   when the rate moved is exactly the mechanism that is missing.

   IT LIVES HERE AND NOT IN A GATE because three sheets need it and a fourth will.
   Three copies of one assertion is how one of them quietly stops asserting it.
   ============================================================================ */

/** Every id in this register, for a caller that wants to check a reference. */
export const CLAIM_IDS = Object.freeze(new Set(CLAIMS.map((c) => c.id)));

/**
 * Refuse a sheet whose cards name a claim id this register does not hold.
 *
 * @param {Array<{id: string, claimIds?: string[]}>} rows  the sheet's cards
 * @param {string} label  the sheet's name, for the error text
 * @returns {number} how many ids were resolved, so a caller can refuse a zero
 *
 * IT THROWS RATHER THAN RETURNING A VERDICT, matching assertCapabilities and the
 * other module floors: a data module that is wrong should stop a build, not hand
 * back a boolean somebody has to remember to read.
 */
export function assertClaimsResolve(rows, label) {
  const named = [...new Set(rows.flatMap((r) => r.claimIds || []))];
  const dangling = named.filter((id) => !CLAIM_IDS.has(id));
  if (dangling.length) {
    throw new Error(
      `${label}: ${dangling.length} of ${named.length} claim id(s) name nothing in ` +
        `src/data/external-claims.js, so the figures beside them age against nothing ` +
        `and no expiry can ever warn:\n` +
        dangling.map((id) => `  - ${id}`).join('\n') +
        `\nEither re-read the vendor and file the claim, or stop publishing the figure.`,
    );
  }
  return named.length;
}
