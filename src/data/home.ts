...
/* THE FAQ NAMES FIGURES NOW, AND NOT ONE OF THEM IS TYPED. `npm run gate`
   ...
   belong. */
import { FLOOR_USD, WEBSITE_USD, RESCUE_AUDIT_DAYS, FLOORS_QUALIFIER } from '../lib/pricing.js';
...
export const proofHead = {
  lede: 'Casino restaurant floor, bank lending division, clinic front office, insurance book, and now the planning floor of the appliance plant in town. Not advising any of them — running them.',
...
export const proofStats: Stat[] = [
  {
    ...
    caption:
      'Run from the inside — a casino restaurant, banking, a clinic, insurance, manufacturing. Every one of them ran on something that almost worked.',
  },
  {
  ...
  },
  {
    ...
    /* THE SAME RETIRED CLAIM, IN THE SAME FILE, IN A THIRD PLACE. This renders
       nowhere today — ProofBar.astro is imported by no page — which is the only
       reason it was not live. A dead copy of a retired claim is the one that
       ships the day somebody wires the component back in. */
    caption: 'Knowing what to build is the hard part — and it’s solved first.',
...
export const process: Step[] = [
  ...
  },
  {
    ...
    /* THE UNDER-AN-HOUR CLAIM CAME OUT 2026-09-13, on the operator's call, and
       the decision to cut it was already made HERE — 130 lines down, the FAQ
       rewrite records retiring "a tool built in under an hour" because "that
       answer was the one place the FAQ asked to be taken on faith". It was cut
       from the FAQ and left standing on /approach/, which is the page that
       explains how the work is done. Same file, same claim, one surface missed.
       "Weeks for a focused tool" stays: it is a number a client can hold him to. */
    body: 'I direct AI to turn that operator insight into real, version-controlled, deployable software — fast. Because the hard part is already solved, the distance between problem and shipped fix collapses: weeks for a focused tool.',
  },
  {
    n: '03',
