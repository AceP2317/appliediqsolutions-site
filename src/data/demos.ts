...
// there is no [slug].astro. The factory demos' manual pages are the exception:
// one shared route, src/pages/work/[slug]/manual.astro, builds all of them.
...
export interface Demo {
   ...
   *  catalog of sheets and lists would otherwise lack. Shelf tools only. */
  instrument?: string;
   ...
   *  catalog having to be organized by trade — grouping by trade tells most
...
export const demos: Demo[] = [
  ...
  },
  {
    ...
    ask: 'How does the Crew Share Calculator handle a trip that cost more than the catch sold for?',
  },
  {
    ...
    seoDescription:
      'A labor cost checker for a small business: your scheduled hours at your own wage rates, against the sales you expect and the target share you set.',
    problem:
  ...
  },
  {
    ...
    ask: 'Does the Reorder Point Calculator forecast demand, or does it just divide what I gave it?',
  },
  {
    slug: 'bar-count-sheet',
    name: 'Bar Inventory Count Sheet',
    instrument: 'Back Bar',
  ...
  },
  {
    ...
    blurb:
      'Labor at your own rates and parts at your own markup, on one estimate, with a call-out minimum that holds and a travel charge that rides on the line it belongs to. Every figure shows the arithmetic beside it, so the number you hand a customer is one you can stand behind at the kitchen table.',
    seoDescription:
      'A job quote builder for the trades: your labor rates, your parts markup, your call-out minimum and travel charge — with the arithmetic printed beside every line.',
    problem:
  ...
  },
  {
    ...
    about:
      'A route business is priced per property and paid for per hour, and those two things never meet on paper. The Lawn Route Pricing Sheet puts them together: what a property pays, divided by the crew time it actually takes — with the drive to it counted, because a twenty-minute drive to a thirty-minute job is a fifty-minute job whatever the invoice says. That single inclusion is usually the whole finding, since the properties that lose money are almost never the slow ones, they are the far ones. It works out the round’s own rate over the same rows the money is summed on rather than averaging the per-property rates, which would weight a ten-minute stop the same as a two-hour one. It also says which properties are past the interval, reusing the same date arithmetic the Recall Due List uses on a patient. It suggests no price, no interval and no rate you should be hitting, and it will never tell you to drop a customer — a property that loses money can still be the reason you got the street, and a page cannot see that.',
    ask: 'Does the Lawn Route Pricing Sheet tell me what to charge, or only what I am getting?',
  },
  {
    slug: 'book-time',
    name: 'Flat Rate Efficiency Sheet',
  ...
  },
  {
    ...
    ask: 'Does the Event Break-Even Calculator predict how busy the festival will be?',
  },
  {
    slug: 'no-show-cost',
    name: 'No-Show Cost Calculator',
    instrument: 'The Day Sheet',
    forWhom: 'Physical therapy, dental, medical — any booked hour',
    tagline: 'What the missed hours cost, on your policy rather than anybody else’s.',
    blurb:
      'A week of visits that did not happen, priced on the practice’s own fee and its own cancellation window. What was not billed and the room you paid for and did not use are reported side by side and never added together, because the lease was paid whether or not the hour was used.',
    seoDescription:
      'A no-show cost calculator: your own fee and window against the visits that did not happen, with the empty room counted beside the money, not into it.',
    problem:
      'Scheduling software will charge the card and then tell you nothing. It cannot know what the visit would have billed, so the one figure a practice actually wants — what the week cost — is the one nothing produces.',
    about:
      'Three things belong to the practice here and to nobody else: how much notice counts as late, whether a late cancel is charged like a no-show, and how much the fee is. This applies those three to the week that actually happened. A no-show gives no notice, so it always falls inside any window; a cancel is late only under the hours you set. A refilled slot recovers whatever the replacement billed, which is often less than the original, so the loss is the difference rather than nothing. The empty room is reported beside the missed billing and deliberately never added to it — that hour was already paid for, and summing the two counts one hour of overhead twice and prints a bigger number than anything that happened. There is no name field and nothing clinical: a booking reference is as far as it goes. It suggests no fee, scores nobody, and compares your week to no published rate.',
    ask: 'Why does the No-Show Cost Calculator refuse to add the empty room to the money?',
  },
  {
    slug: 'provider-split',
    name: 'Provider Production Split',
    instrument: 'The Month End',
    forWhom: 'Dental, medical and veterinary group practices',
    tagline: 'What each provider is owed under the agreement they actually signed.',
    blurb:
    ...
    seoDescription:
      'A provider compensation calculator for a practice: each associate’s own percentage and basis, production or collection, with lab off the top or not.',
    problem:
    ...
    ask: 'Why will the Provider Production Split not divide collection by production?',
  },
  {
    slug: 'claim-aging',
    name: 'Claim Aging by Payer',
    instrument: 'The Remittance',
    forWhom: 'Medical, dental and behavioral health billing',
    tagline: 'Which claims are past the interval you set for that payer.',
    blurb:
    ...
    seoDescription:
      'An insurance claim aging worksheet: your own follow-up interval per payer against what each one still owes, sorted by how far past it each claim is.',
    problem:
    ...
    ask: 'Why does the Claim Aging by Payer refuse to draw thirty, sixty and ninety day buckets?',
  },
  {
    slug: 'tuition-sheet',
    name: 'Childcare Tuition Sheet',
    instrument: 'The Sign-In',
    forWhom: 'Childcare centers, preschools, after-school programs',
    tagline: 'What each family owes, on your rate card and your sibling rule.',
    blurb:
    ...
    seoDescription:
      'A childcare tuition calculator: your own weekly and day rates, your sibling discount and which place it comes off, vouchers, and your late-pickup rate.',
    problem:
    ...
    ask: 'Why will the Childcare Tuition Sheet not work out a staffing ratio?',
  },
  {
    slug: 'markdown-margin',
    name: 'Markdown Margin Check',
    instrument: 'The Sticker',
    forWhom: 'Shops, chandleries, boutiques — anything on a rail',
    tagline: 'What the sale does to the margin, against the floor you set.',
    blurb:
    ...
    seoDescription:
      'A markdown and margin calculator for a shop: your cost, your ticket price and your own margin floor, with the markup shown beside the margin.',
    problem:
    ...
    ask: 'Why will the Markdown Margin Check not tell me what a markdown cost me?',
  },
  {
    slug: 'consignment',
    name: 'Consignment Payout',
    instrument: 'The Contract Card',
    forWhom: 'Consignment shops, resale, vintage, antique malls',
    tagline: 'What each consignor is owed, and what the shop actually kept.',
    blurb:
    ...
    seoDescription:
      'A consignment payout calculator: your split per consignor, your price band, your card fee and your term, with the shop’s share taken from what actually arrived.',
    problem:
    ...
    ask: 'Why does the Consignment Payout work out the shop’s share by subtraction instead of by percentage?',
  },
  {
    slug: 'drawer-count',
    name: 'Register Drawer Count',
    instrument: 'The Till Tape',
    forWhom: 'Shops, markets, bars, anywhere a drawer gets counted',
    tagline: 'What the till says against what is in the drawer, on your tolerance.',
    blurb:
      'Every point-of-sale system can subtract what the till expected from what was counted. None of them holds how far out a drawer may be before somebody stops and looks — which is a cash floor and a share of what was rung, whichever is larger, and both halves are the shop’s own.',
    seoDescription:
      'A cash drawer count sheet: your float and your own over-and-short tolerance, counted by denomination so a drawer that balances but cannot open is visible.',
    problem:
      'The subtraction is trivial and it still gets skipped, because the number it produces means nothing without a tolerance — and the tolerance lives in whoever is closing, not in the system.',
    about:
      'It holds no names, there is no field for who was on the register, and there will not be one. A short drawer is a fact about a drawer, and turning it into a fact about a person is a different product and a worse one — it is also the thing that stops people typing honest counts in. The tolerance is two numbers rather than one, because a dollar out on a forty-dollar evening is a different thing from a dollar out on a nine-hundred-dollar Saturday, so a cash floor and a share of what was rung both apply and the larger one wins. It is applied per drawer and never to the total, since one over and one short cancel to nothing and are still two problems. And the count is held by denomination rather than as a figure, because a drawer can balance perfectly and still be unable to open tomorrow — the right money in the wrong notes, which no single total can show.',
    ask: 'Why does the Register Drawer Count refuse to record who was on the register?',
  },
  {
    slug: 'turn-cost',
    name: 'Room Turn Cost',
    instrument: 'The Housekeeping Card',
    forWhom: 'B&Bs, inns, short-term rentals, holiday cottages',
    tagline: 'What a turn costs, and what that is a night once people stay longer.',
    blurb:
      'A turn costs the same whatever the length of the stay — the linens, the hour of housekeeping and what gets left out do not know how many nights were booked. So the figure per stay holds still and the figure per night does not, and both are here.',
    seoDescription:
      'A cleaning and turnover cost calculator for a short-term rental: your housekeeping rate, your linens and consumables, per stay and per night.',
    problem:
      'The turn cost lives in one place and the length of stay lives in another, so nobody ever divides one by the other. The room with the cheapest turn is often the dearest to run, and no booking report will ever say so.',
    about:
      'What a turn includes is a standard this house set: how long it takes in each room, what goes on the bed, what is left out for a guest, how often the things that are not replaced every stay actually get replaced, and what is held back against a deep clean. The things restocked every few stays are spread across the stays they cover, because charging a bottle that lasts six turns to one of them overstates that stay and understates five, and leaving it out is how a turn cost ends up a third light. The finding is the second figure rather than the first. Cost per stay is fixed; cost per night falls as the stay lengthens, and it falls fast. On the sample the cheapest turn is the dearest per night, because it turns twice as often. That is also why a minimum-stay rule is a cost decision before it is a marketing one — a longer stay does not make a turn cheaper, it spreads the same turn over more nights, which is a different sentence and a far more useful one.',
    ask: 'Why does the Room Turn Cost print a cost per night as well as a cost per stay?',
  },
  {
    slug: 'night-net',
    name: 'Net Per Night',
    instrument: 'The Folio',
    forWhom: 'B&Bs, inns, short-term rentals, holiday cottages',
    tagline: 'What is left of a booking once everything has come off it, a night at a time.',
    blurb:
      'The platform prints its commission on every payout. What it cannot print is what a turn costs this house, or which side of the occupancy tax the booking fell on — and between them those two decide most of what a night actually keeps.',
    seoDescription:
      'A short-term rental net calculator: your platform rates, your occupancy tax and your own turn cost, worked down to what one night actually keeps.',
    problem:
      'A payout statement shows the commission and stops. The turn comes off later, the tax was handled by somebody else, and the cleaning fee was set once years ago — so nothing anywhere says what a night is worth.',
    about:
      'A platform’s commission is the same for every host on it, so a tool built on that number would be doing arithmetic the payout already shows you. The two figures that are the host’s own are what a turn costs this house and how the occupancy tax was handled, and both are invisible on every statement. Tax a platform collects on top and remits is not income at all — it never enters gross here and never enters what is kept — while tax the host collected is money they owe, and counting the first as revenue is the most common way this sum goes wrong. The finding is the cleaning fee against the turn. A fee is set once in a listing; a turn cost is worked out never, so the two have no relationship and the gap is invisible. Charge over the turn and the surplus is quietly subsidizing the nightly rate. Charge under it and the house is absorbing part of every turn, on every booking, with no line item anywhere saying so. Both directions are printed beside the net.',
    ask: 'Why does Net Per Night keep the occupancy tax a platform remitted out of the gross entirely?',
  },
  {
    slug: 'rent-roll',
    name: 'Rent Roll and Owner Statement',
    instrument: 'The Statement',
    forWhom: 'Property managers, letting agents, small landlords',
    tagline: 'What actually goes out to each owner, once your fee, their repairs and their reserve are off it.',
    blurb:
    ...
    seoDescription:
      'A rent roll and owner statement calculator: your fee on rent charged or collected, deductions and reserves, down to what each owner is actually paid.',
    problem:
    ...
    ask: 'Why does the Rent Roll print a management fee on rent collected as well as on rent charged?',
  },
  {
    slug: 'unit-turn',
    name: 'Unit Turn and Vacancy Cost',
    instrument: 'The Punch List',
    forWhom: 'Landlords, property managers, letting agents',
    tagline: 'What a turn between tenants actually cost, which is not what the contractor invoiced.',
    blurb:
    ...
    seoDescription:
      'A unit turn and vacancy cost calculator: make-ready against the rent lost while empty, with the vacant days split into the work and the market.',
    problem:
    ...
    ask: 'Why does the Unit Turn sheet split the vacant days into days to ready and days on market?',
  },
  {
    slug: 'retainage',
    name: 'Retainage Held',
    instrument: 'The Pay Application',
    forWhom: 'Subcontractors, specialty trades, small general contractors',
    tagline: 'What you have earned, what you have been paid, and what is still being held.',
    blurb:
    ...
    seoDescription:
      'A retainage tracker for trades: what is held against each contract, what the contract allows, and which jobs are past their own release condition.',
    problem:
    ...
    ask: 'How does Retainage Held decide which contracts are past their release condition?',
  },
  {
    slug: 'draw-schedule',
    name: 'Draw Schedule Tracker',
    instrument: 'The Draw Request',
    forWhom: 'General contractors, remodelers, and builders on financed jobs',
    tagline: 'Which stages are finished, and which of those nobody has drawn against.',
    blurb:
    ...
    seoDescription:
      'A construction draw schedule tracker: every stage against the work done, which finished stages were never drawn, and whether the schedule foots to the contract.',
    problem:
    ...
    ask: 'How does the Draw Schedule Tracker find a stage that was finished and never drawn against?',
  },
  {
    slug: 'billable-hours',
    name: 'Billable Hours Realization',
    instrument: 'The Timesheet',
    forWhom: 'Law firms, accounting practices, agencies, and independent consultants',
    tagline: 'Where the gap goes between an hour worked and a dollar collected.',
    blurb:
    ...
    seoDescription:
      'A billable hours realization sheet: what was lost at billing, what was lost at collection, and which matters carry hours nobody has invoiced.',
    problem:
    ...
    ask: 'How does Billable Hours Realization tell money lost at billing from money lost at collection?',
  },
  {
    slug: 'retainer-burn',
    name: 'Retainer Burn',
    instrument: 'The Retainer Ledger',
    forWhom: 'Agencies, consultants, bookkeepers, and IT support shops on monthly retainers',
    tagline: 'Hours they already paid for, about to disappear at the close.',
    blurb:
    ...
    seoDescription:
      'A retainer ledger: which clients have hours they paid for that expire at the close, what those hours are worth, and what the fixed fee came to an hour.',
    problem:
    ...
    ask: 'How does Retainer Burn work out which unused hours expire at the close of the period?',
  },
  {
    slug: 'boarding-rate',
    name: 'Boarding and Grooming Day Rate',
    instrument: 'The Kennel Card',
    forWhom: 'Boarding kennels, doggy daycare, groomers, and pet sitters',
    tagline: 'What a stay comes to, once the peak nights and the add-ons are counted in.',
    blurb:
      'Enter each stay, the nights you call peak, and your own rates. This splits every booking into peak nights and standard nights with the money each side carries, totals the baths and nail trims and medication beside the board, takes the sibling discount off a shared run, and prints what each stay comes to and what is still owed on it.',
    seoDescription:
      'A kennel day-rate sheet: which nights of a stay are peak, what the peak rule adds, what the add-ons come to beside board, and what is still owed.',
    problem:
      'A holiday stay and a stay a week later are the same length on a booking sheet and price completely differently, and the baths and nail trims are quoted one at a time and never totaled against board — so nobody knows what either one is actually worth.',
    about:
      'Two rules set the price of a stay and both of them are the owner’s own. Which nights are peak is a list somebody wrote on the office wall, and it is different at every kennel in the county; what a shared run is worth is a second one. Nothing a kennel owns applies either rule, so a booking sheet shows dates and a calendar shows days, and neither shows that the same six nights price completely differently across a holiday weekend. This reads one card per stay and prints two things. First, every booking split into peak nights and standard nights, with the money each side carries and what the same nights would have come to at the base rate on every one of them — the gap between those two figures is the peak rule, in dollars. Second, the add-ons totaled beside the board with the share they come to, because baths, nail trims and medication are a second business running inside the first and nobody adds them up. It suggests no nightly rate, names no day peak, and prices no add-on.',
    ask: 'How does the Boarding and Grooming Day Rate sheet decide which nights of a stay are peak?',
  },
  {
 ...
 * is the supply-chain work. Until D118 (2026-09-20) `scale` was PROOF and never
 * the offer (D29); it is now the factory tools a manufacturer buys, built to
 * order per plant and sold on /supply-chain/. The Work index renders one section per
...
export interface WorkGroup {
   ...
   *  numbers, and that nothing is modeled. And its category closed in March
...
export const workGroups: WorkGroup[] = [
  {
    ...
    band: 'local',
    slugs: ['bar-count-sheet', 'schedule-cost-check', 'marina-storage', 'reorder-list', 'property-round', 'drawer-count'],
  },
  {
    label: 'The rule that is yours',
    band: 'local',
    slugs: ['tip-out-sheet', 'crew-share', 'commission-check', 'chair-split', 'provider-split', 'tuition-sheet', 'consignment', 'rent-roll', 'retainer-burn'],
  },
     ...
     would hide the one thing a contractor is scanning the page for. */
  {
    ...
    band: 'local',
    slugs: ['job-quote', 'event-day', 'markdown-margin', 'boarding-rate'],
  },
  {
  ...
  /* ITS OWN GROUP, and the shape is the opposite of "Money going out the door"
     rather than a variation on it. Everything in that group is spend a business
     can see on a statement. This is revenue that never arrived — a visit nobody
     billed, a claim nobody chased — so there is no line anywhere to look at, and
     that absence is exactly why nothing gets worked out. */
  {
    label: 'Money that never arrived',
    band: 'local',
    slugs: ['no-show-cost', 'claim-aging', 'draw-schedule'],
  },
  {
     ...
     this group's at-scale twin is obvious from the name alone. */
  {
    ...
    band: 'local',
    slugs: ['book-time', 'turn-cost', 'night-net', 'unit-turn', 'retainage', 'billable-hours'],
  },
  {
