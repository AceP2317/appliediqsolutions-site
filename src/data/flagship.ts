/**
 * Confirmation Outlook — a deployed service, not a browser demo.
 *
 * Deliberately NOT in demos.ts: those are client-side React islands at
 * /work/<slug>/, and every consumer (LiveTools, work/index, ShipLog, demoSchema,
 * llms.txt) assumes that contract. This one is a Python service on a server.
 * Different claim, different surface — and `toolCount` stays honest.
 *
 * The file and export are still named `flagship`: they are internal identifiers
 * across several files, not copy. The rendered WORD "flagship" retired with D29
 * — this is proof of what gets built, not the offer — but the honesty contract
 * below is not a function of the label and does not move with it.
 *
