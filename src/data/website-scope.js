/* ============================================================================
   ...
   account. D96 carries the reasoning.

   THE COUNT IS NOT IN THIS COMMENT AND MUST NOT COME BACK — it read "the six
   things" through six versions of this file and was wrong the moment a seventh
   landed, which is D22's rule applied to a sentence rather than to a page.
   Derive it from `countWord`/`forgottenWord` below.

   AND ON 2026-09-13 THAT RULE WAS FOUND TO HAVE BEEN WRITTEN AND NOT ENFORCED.
   The paragraph above removed the count from ITSELF and left it typed in five
   other places, two of them in this same header, one of them the rendered h2 on
   /websites/ — which told a reader "Six things" above a grid of seven — and one
   of them the printed comparison sheet a client is handed. A rule that lives in
   the comment of the file it governs is a rule with a readership of one.

   THE SCOPE IS THE ARGUMENT FOR THE NUMBER, so this list and that document must
   not drift — all but one of these were never priced into the published shelf
   ...
   comparison is one of the things the floor is argued from — which is why
...
/** How many things a build includes, spelled out for prose.
 *  @param {{capital?: boolean}} [opts] */
export const countWord = (opts) => spell(included.length, opts);

/** How many of them a buyer would not have thought to ask for — every one but
 *  the site itself, which is the only item nobody forgets.
 *
 *  IT IS DERIVED RATHER THAN TYPED FOR THE SAME REASON THE TOTAL IS, and the
 *  sentence it lands in is why it needs its own export: "N things, and N-1 of
 *  them are the ones people forget" carries the count TWICE, so a hand-typed
 *  version has two chances to go wrong and went wrong on both.
 *  @param {{capital?: boolean}} [opts] */
export const forgottenWord = (opts) => spell(included.length - 1, opts);
