...
/* ── 1. The comet ─────────────────────────────────────────────────────────────
   ...
   toward the one in front of it. That single rule produces all the behavior:
...
if (fine && !reduced) {
  ...
  const dots = Array.from({ length: COUNT }, (_, i) => {
    ...
    d.style.marginLeft = `${-size / 2}px`;   // center on the point
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      lastX = px;
      lastY = py;
      // Normalized so a brisk flick reaches ~1 and a slow drift stays dim.
...
/* ── 2b. Tap ripple (touch) ───────────────────────────────────────────────────
   ...
   acknowledgment instead: a soft depression with a ring traveling outward.
...
/* ── The currents run ─────────────────────────────────────────────────────────
   ...
   two `.confluence` rules in global.css, so what it actually does is push both
   currents of the confluence backdrop hot for a beat. The name is kept because
   renaming it means touching the stylesheet, the markup and this file at once
   for no behavior change — but nothing it does is a synapse any more. */
let fireTimer = null;
...
/* ── 4. The wordmark tittle ───────────────────────────────────────────────────
   ...
   sends the backdrop's currents running. The <a> around the wordmark still
...
console.log(
  '%c  Applied%ciQ%c  ',
  /* Retired palette and a font this site no longer loads. Updated 2026-08-24:
     Space Grotesk is gone, so this fell back to whatever sans the browser had,
     and the two hexes were the old slate theme. A console message is styled
     against DEVTOOLS, which is usually dark, so it keeps a dark ground on
     purpose rather than following the page. */
  'background:#17150f;color:#faf7f2;font:600 20px/2.2 Fraunces,Georgia,serif',
  'background:#17150f;color:#43918a;font:600 20px/2.2 Fraunces,Georgia,serif',
  'background:#17150f',
);
console.log(
  ...
  'color:#2f6f6a;font:13px/1.6 ui-sans-serif,system-ui',
);
