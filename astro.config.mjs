...
import { date } from './src/lib/format.js';

/* THE BUILD STAMP the factory demos print in their footer: the build date in ET
   and a short commit code, so a live copy that has fallen behind its saved code
   says so on screen (house rule; MD04's live copy sat one change behind on
   2026-09-23 with nothing showing it).

   Git first, then the commit variable each build host sets, and only then
   "unstamped build" — which is printed rather than left blank, because a blank
   stamp reads like a current one. The fallback WARNS here, at build time: a
   quiet fallback is a defect on sight. */
function buildStamp() {
  let sha = '';
  try {
    sha = execSync('git rev-parse --short HEAD', { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
  } catch {
    sha = (process.env.WORKERS_CI_COMMIT_SHA || process.env.CF_PAGES_COMMIT_SHA || process.env.GITHUB_SHA || '').slice(0, 7);
  }
  if (!sha) {
    console.warn('[build-stamp] no git and no build-host commit variable — the factory demos will print "unstamped build".');
    return 'unstamped build';
  }
  return `Built ${date(new Date())} · ${sha}`;
}
...
export default defineConfig({
  ...
  // that Astro 6 kept. The move to 7 (2026-09-25) was for security fixes, not a new look, so the
  // 6.x behaviour is kept, and a pixel comparison of every page before and after confirmed it.
  compressHTML: true,
  ...
  integrations: [
    ...
    // number), and /meet and /host are the client meeting room reached only by
    // a link Ian sends — keep them out of the sitemap. No page carries a
    // lastmod: the one that changed daily, /ai-news/, came down on 2026-09-20
    // (D118), and a build-time lastmod on every URL trains crawlers to distrust
    // the signal.
    sitemap({
      changefreq: 'monthly',
      priority: 0.7,
  ...
  vite: {
    plugins: [tailwindcss()],
    define: { __NP_BUILD_STAMP__: JSON.stringify(buildStamp()) },
